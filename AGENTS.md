...
当前 Phase 1 仅允许：Workspace / Task / Session 的最小持久化关系、本地 Markdown/Text/PDF/DOCX 导入、源路径与哈希及 Locator 记录、SQLite FTS5 检索、只读 Knowledge Search Tool、由检索结果生成的本地 Evidence 及对应类型化 IPC/UI。XLSX/PPTX 解析、Embedding、Web Research、Artifact 与完整研究 Agent 仍须在后续切片单独实现。
