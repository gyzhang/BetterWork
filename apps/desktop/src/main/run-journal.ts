...
interface EvidenceRow {
  ...
  task_id: string;
  run_id: string;
  source_type: 'local-file' | 'web-page';
  source_uri: string;
  title: string;
  locator: string;
  excerpt: string;
...
interface NotificationRow {
  id: string;
  level: NotificationSummary['level'];
  kind: NotificationSummary['kind'];
  title: string;
  detail: string | null;
...
const parseSearchOptions = (raw: unknown): { webTopK: number } => {
  try {
    ...
    return Number.isInteger(webTopK) && webTopK >= 1 && webTopK <= 20
      ? { webTopK }
      : { webTopK: 10 };
  } catch {
    return { webTopK: 10 };
...
export class RunJournal {
  ...
  constructor(filePath: string) {
    ...
    `);
    const columns = this.db.prepare('PRAGMA table_info(model_profiles)').all() as Array<{
      name: string;
    }>;
    if (!columns.some((column) => column.name === 'connection_status'))
      this.db.exec(
        "ALTER TABLE model_profiles ADD COLUMN connection_status TEXT NOT NULL DEFAULT 'untested'",
      );
    if (!columns.some((column) => column.name === 'last_tested_at'))
      this.db.exec('ALTER TABLE model_profiles ADD COLUMN last_tested_at INTEGER');
    const artifactVersionColumns = this.db
      .prepare('PRAGMA table_info(artifact_versions)')
      .all() as Array<{ name: string }>;
    if (!artifactVersionColumns.some((column) => column.name === 'origin'))
      this.db.exec(
  ...
  saveLocalEvidence(input: Omit<EvidenceSummary, 'id' | 'sourceType' | 'capturedAt'>): void {
    this.db
      .prepare(
        `INSERT OR IGNORE INTO evidence (id, task_id, run_id, source_type, source_uri, title, locator, excerpt, content_hash, captured_at) VALUES (?, ?, ?, 'local-file', ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        randomUUID(),
        input.taskId,
  ...
  saveWebEvidence(input: Omit<EvidenceSummary, 'id' | 'sourceType' | 'capturedAt'>): void {
    this.db
      .prepare(
        `INSERT OR IGNORE INTO evidence (id, task_id, run_id, source_type, source_uri, title, locator, excerpt, content_hash, captured_at) VALUES (?, ?, ?, 'web-page', ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        randomUUID(),
        input.taskId,
  ...
  saveMarkdownArtifact(input: SaveMarkdownArtifactRequest): ArtifactSummary {
    ...
    const existing = input.artifactId
      ? (this.db
          .prepare(
            'SELECT id, workspace_id, task_id, current_version_id FROM artifacts WHERE id = ?',
          )
          .get(input.artifactId) as
          | { id: string; workspace_id: string; task_id: string; current_version_id: string }
          | undefined)
      : undefined;
    if (
      input.artifactId &&
    ...
    this.db.transaction(() => {
      if (existing) {
        ...
        this.db
          .prepare(
            'INSERT INTO artifacts (id, workspace_id, task_id, type, title, current_version_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          )
          .run(
            artifactId,
            task.workspace_id,
      ...
      this.db
        .prepare(
          'INSERT INTO artifact_versions (id, artifact_id, version_number, content, content_hash, source_run_id, origin, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .run(
          versionId,
          artifactId,
      ...
      if (input.origin === 'assistant-run')
        this.db
          .prepare(
            'INSERT OR IGNORE INTO artifact_version_evidence (version_id, evidence_id) SELECT ?, id FROM evidence WHERE task_id = ? AND run_id = ?',
          )
          .run(versionId, input.taskId, input.runId);
      else if (existing)
        this.db
          .prepare(
            'INSERT OR IGNORE INTO artifact_version_evidence (version_id, evidence_id) SELECT ?, evidence_id FROM artifact_version_evidence WHERE version_id = ?',
          )
  ...
  listArtifacts(taskId?: string): ArtifactSummary[] {
    const rows = taskId
      ? this.db
          .prepare(
            `SELECT a.id, a.workspace_id, a.task_id, a.type, a.title, a.current_version_id, v.version_number, v.source_run_id, v.origin, a.created_at, a.updated_at FROM artifacts a JOIN artifact_versions v ON v.id = a.current_version_id WHERE a.task_id = ? ORDER BY a.updated_at DESC, a.rowid DESC`,
          )
          .all(taskId)
      : this.db
          .prepare(
            `SELECT a.id, a.workspace_id, a.task_id, a.type, a.title, a.current_version_id, v.version_number, v.source_run_id, v.origin, a.created_at, a.updated_at FROM artifacts a JOIN artifact_versions v ON v.id = a.current_version_id ORDER BY a.updated_at DESC, a.rowid DESC`,
          )
          .all();
  ...
  getArtifactDetail(id: string): ArtifactDetail | undefined {
    const row = this.db
      .prepare(
        `SELECT a.id, a.workspace_id, a.task_id, a.type, a.title, a.current_version_id, v.version_number, v.source_run_id, v.origin, v.content, v.content_hash, a.created_at, a.updated_at FROM artifacts a JOIN artifact_versions v ON v.id = a.current_version_id WHERE a.id = ?`,
      )
      .get(id) as (ArtifactRow & { content: string; content_hash: string }) | undefined;
    return row
      ? {
          ...this.toArtifactSummary(row),
          content: row.content,
          contentHash: row.content_hash,
          evidence: this.listArtifactVersionEvidence(row.current_version_id),
        }
      : undefined;
  }
  ...
  listArtifactVersions(artifactId: string): ArtifactVersionSummary[] {
    const rows = this.db
      .prepare(
        'SELECT id, artifact_id, version_number, source_run_id, origin, created_at FROM artifact_versions WHERE artifact_id = ? ORDER BY version_number DESC',
      )
  ...
  getArtifactVersionDetail(id: string): ArtifactVersionDetail | undefined {
    const row = this.db
      .prepare(
        'SELECT id, artifact_id, version_number, source_run_id, origin, content, content_hash, created_at FROM artifact_versions WHERE id = ?',
      )
      .get(id) as ArtifactVersionRow | undefined;
    return row && row.content !== undefined && row.content_hash !== undefined
      ? {
          ...this.toArtifactVersionSummary(row),
          content: row.content,
          contentHash: row.content_hash,
          evidence: this.listArtifactVersionEvidence(row.id),
        }
      : undefined;
  }
  ...
  listRuns(taskId?: string): RunSummary[] {
    const rows = (
      taskId
        ? this.db
            .prepare(
              'SELECT * FROM runs WHERE task_id = ? ORDER BY created_at DESC, rowid DESC LIMIT 100',
            )
            .all(taskId)
        : this.db.prepare('SELECT * FROM runs ORDER BY created_at DESC, rowid DESC LIMIT 100').all()
    ) as RunRow[];
    return rows.map((row) => ({
      id: row.id,
      taskId: row.task_id,
  ...
  getModelForRun(role: ModelProfileSummary['role']):
    | {
        id: string;
        baseUrl: string;
        apiKey: string;
    ...
    const row = this.db
      .prepare(
        'SELECT id, base_url, api_key, model, temperature, max_output_tokens FROM model_profiles WHERE role = ? AND enabled = 1 ORDER BY priority ASC LIMIT 1',
      )
  ...
  saveModel(input: ModelProfileInput & { id?: string | undefined }): string {
    ...
    const existing = this.db
      .prepare('SELECT id, created_at FROM model_profiles WHERE id = ?')
    ...
    if (existing) {
      const apiKey = input.apiKey
        ...
        : String(
            (
              this.db.prepare('SELECT api_key FROM model_profiles WHERE id = ?').get(id) as {
                api_key: string;
              }
            ).api_key,
          );
      this.db
        .prepare(
          `UPDATE model_profiles SET name=?, provider=?, base_url=?, model=?, role=?, api_key=?, enabled=?, priority=COALESCE(?, priority), max_context_tokens=?, max_output_tokens=?, temperature=?, connection_status='untested', last_tested_at=NULL, updated_at=? WHERE id=?`,
        )
        .run(
          input.name,
          input.provider,
      ...
      this.db
        .prepare(
          `INSERT INTO model_profiles (id,name,provider,base_url,model,role,api_key,enabled,priority,max_context_tokens,max_output_tokens,temperature,connection_status,last_tested_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        )
        .run(
          id,
          input.name,
  ...
  setDefaultModel(id: string): boolean {
    const target = this.db
      ...
      .get(id) as { role: string; enabled: number } | undefined;
  ...
  saveSearchEngine(input: SearchEngineConfigInput): string {
    ...
    const existing = this.db
      .prepare(
        'SELECT api_key, connection_status, last_tested_at FROM search_engine_configs WHERE provider = ?',
      )
      .get(input.provider) as
      { api_key: string; connection_status: string; last_tested_at: number | null } | undefined;
    const transaction = this.db.transaction(() => {
      ...
      if (existing) {
        this.db
          .prepare(
            'UPDATE search_engine_configs SET api_key = ?, options = ?, enabled = ?, connection_status = ?, last_tested_at = ?, updated_at = ? WHERE provider = ?',
          )
          .run(
            apiKey,
        ...
        this.db
          .prepare(
            'INSERT INTO search_engine_configs (provider, api_key, options, enabled, connection_status, last_tested_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          )
          .run(
            input.provider,
            apiKey,
  ...
  getEnabledSearchEngine():
    { provider: SearchProviderId; apiKey: string; webTopK: number } | undefined {
    const row = this.db
      .prepare(
        'SELECT provider, api_key, options FROM search_engine_configs WHERE enabled = 1 LIMIT 1',
      )
      .get() as Record<string, unknown> | undefined;
    if (!row) return undefined;
    return {
      provider: String(row.provider) as SearchProviderId,
      apiKey: String(row.api_key ?? ''),
  ...
  getSearchEngine(
    ...
    const row = this.db
      .prepare('SELECT provider, api_key, options FROM search_engine_configs WHERE provider = ?')
  ...
  saveNotification(input: CreateNotificationInput): NotificationSummary {
    const row: NotificationRow = {
      ...
      target_id:
        input.target && 'taskId' in input.target
          ? input.target.taskId
          : input.target && 'artifactId' in input.target
    ...
    const transaction = this.db.transaction(() => {
      this.db
        .prepare(
          'INSERT INTO notifications (id, level, kind, title, detail, target_kind, target_id, read, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .run(
          row.id,
          row.level,
      ...
      this.db
        .prepare(
          'DELETE FROM notifications WHERE rowid NOT IN (SELECT rowid FROM notifications ORDER BY created_at DESC, rowid DESC LIMIT 200)',
        )
        .run();
    });
    transaction();
  ...
  listNotifications(): NotificationSummary[] {
    const rows = this.db
      .prepare('SELECT * FROM notifications ORDER BY created_at DESC, rowid DESC LIMIT 200')
  ...
  unreadNotificationCount(): number {
    return (
      this.db.prepare('SELECT COUNT(*) AS count FROM notifications WHERE read = 0').get() as {
        count: number;
      }
    ).count;
  }
  ...
  private toNotificationSummary(row: NotificationRow): NotificationSummary {
    ...
    if (row.target_kind === 'task' && row.target_id)
      target = { kind: 'task', taskId: row.target_id };
    else if (row.target_kind === 'artifact' && row.target_id)
      target = { kind: 'artifact', artifactId: row.target_id };
  ...
  private toModelSummary(row: Record<string, unknown>): ModelProfileSummary {
    return {
      ...
      baseUrl: String(row.base_url),
      model: String(row.model),
      role: row.role as ModelProfileSummary['role'],
      ...
      ...(row.last_tested_at === null || row.last_tested_at === undefined
        ? {}
        : { lastTestedAt: Number(row.last_tested_at) }),
  ...
  private getArtifact(id: string): ArtifactSummary | undefined {
    const row = this.db
      .prepare(
        `SELECT a.id, a.workspace_id, a.task_id, a.type, a.title, a.current_version_id, v.version_number, v.source_run_id, v.origin, a.created_at, a.updated_at FROM artifacts a JOIN artifact_versions v ON v.id = a.current_version_id WHERE a.id = ?`,
      )
  ...
  private listArtifactVersionEvidence(versionId: string): EvidenceSummary[] {
    const rows = this.db
      .prepare(
        'SELECT e.* FROM artifact_version_evidence ave JOIN evidence e ON e.id = ave.evidence_id WHERE ave.version_id = ? ORDER BY e.captured_at DESC, e.rowid DESC',
      )
