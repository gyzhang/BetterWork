...
function KnowledgePage({ documents, results, query, setQuery, message, importing, onImport, onSearch }: { documents: KnowledgeDocumentSummary[]; results: KnowledgeSearchResult[]; query: string; setQuery: (query: string) => void; message: string; importing: boolean; onImport: () => void; onSearch: (event: FormEvent) => void }): React.JSX.Element {
  ...
  return <section className="knowledge-page"><header><div><p className="eyebrow">知识 · 个人资料库</p><h1>让资料成为下一次工作的起点</h1><p>资料保留在你的本机路径；算台只建立可重建的本地文本索引。当前支持 Markdown、文本与 PDF。</p></div><button className="primary-button" disabled={importing} onClick={onImport}>{importing ? '正在导入…' : '＋ 导入资料'}</button></header><form className="knowledge-search" onSubmit={onSearch}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索资料库中的内容…" aria-label="搜索个人资料库" /><button type="submit">搜索</button>{showingResults && <button type="button" className="clear-search" onClick={() => setQuery('')}>清除</button>}</form>{message && <p className="inline-message">{message}</p>}<div className="knowledge-summary"><span>{showingResults ? `找到 ${items.length} 条相关资料` : `已整理 ${documents.length} 份资料`}</span><small>{showingResults ? '检索仅在本地资料库中进行' : '下一步将支持 Office 与语义检索'}</small></div>{items.length === 0 ? <EmptyPage eyebrow={showingResults ? '没有匹配结果' : '从一份资料开始'} title={showingResults ? '换个关键词试试' : '把常用资料放进你的资料库'} detail={showingResults ? '当前先按文本内容进行本地检索。' : '导入 Markdown、文本或 PDF 后，它们会在后续研究和写作中成为可引用的个人资料。'} /> : <div className="knowledge-list">{items.map(({ document, excerpt, locator }) => <article className="knowledge-card" key={`${document.id}-${locator ?? 'document'}`}><span className={`knowledge-format ${document.format}`}>{document.format === 'markdown' ? 'MD' : document.format === 'pdf' ? 'PDF' : 'TXT'}</span><div><strong>{document.title}</strong>{excerpt && <p>{excerpt}</p>}<small>{document.sourcePath}{locator ? ` · ${locator}` : ''} · 更新于 {formatTime(document.updatedAt)}</small></div></article>)}</div>}</section>;
}
