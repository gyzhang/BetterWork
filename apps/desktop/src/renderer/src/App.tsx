...
const eventTitle = (event: AgentRuntimeEvent): string => ({
  'run.started': '开始处理任务', 'message.started': '正在组织回复', 'message.delta': '正在撰写', 'message.completed': '回复已完成',
  'reasoning.delta': '正在判断下一步', 'tool.requested': '准备使用工具', 'tool.started': '正在执行工具', 'tool.progress': '工具处理中',
  'tool.completed': '工具执行完成', 'tool.failed': '工具执行失败', 'run.completed': '任务已完成', 'run.failed': '任务失败', 'run.cancelled': '任务已停止',
})[event.type];
...
export function App(): React.JSX.Element {
  ...
  return <main className="app-shell">
    ...
    <section className="main-stage">
      ...
      {view === 'artifacts' && <EmptyPage eyebrow="成果" title="让成果成为下一步工作的起点" detail="研究报告、数据分析、Word 文档与演示将在对应工作流完成后集中管理。" />}
      {view === 'knowledge' && <EmptyPage eyebrow="知识" title="你的资料，正在等待被组织" detail="本地知识库将在下一阶段接入；资料仍将由你在本机管理。" />}
      {view === 'settings' && <SettingsPage tab={settingsTab} setTab={setSettingsTab} models={filteredModels} modelFilter={modelFilter} setModelFilter={setModelFilter} activeLanguageModel={activeLanguageModel} onAdd={() => openModelEditor()} onEdit={openModelEditor} onToggle={toggleModel} onDelete={(model) => void window.betterwork.models.delete({ id: model.id }).then(refreshModels)} appearance={appearance} resolvedAppearance={resolvedAppearance} onMode={(mode) => setAppearanceValue({ ...appearance, mode })} onScheme={(scheme) => setAppearanceValue({ ...appearance, scheme })} modelMessage={modelMessage} />}
    </section>
    {view === 'work' && <ContextPanel open={contextOpen} setOpen={setContextOpen} tab={contextTab} setTab={setContextTab} events={events} activeRun={activeRun} isRunning={isRunning} completedTools={completedTools.length} />}
...
function ContextPanel({ open, setOpen, tab, setTab, events, activeRun, isRunning, completedTools }: { open: boolean; setOpen: (open: boolean) => void; tab: ContextTab; setTab: (tab: ContextTab) => void; events: AgentRuntimeEvent[]; activeRun?: RunSummary | undefined; isRunning: boolean; completedTools: number }): React.JSX.Element {
  if (!open) return <aside className="context-collapsed"><button aria-label="展开上下文面板" onClick={() => setOpen(true)}>◀</button></aside>;
  return <aside className="context-panel"><div className="context-topline"><span>当前任务</span><button aria-label="收起上下文面板" onClick={() => setOpen(false)}>›</button></div><div className="context-tabs" role="tablist" aria-label="任务上下文">{([['process', '过程'], ['sources', '资料'], ['artifacts', '成果']] as const).map(([key, label]) => <button key={key} role="tab" aria-selected={tab === key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}</button>)}</div><div className="context-content">{tab === 'process' && (events.length === 0 ? <EmptyContext title="等待任务开始" detail="开始后，这里会按工作阶段呈现过程，而不是堆叠底层日志。" /> : <div className="activity-list"><div className="activity-summary"><span className={isRunning ? 'status-dot running' : 'status-dot'} /><div><strong>{isRunning ? '正在处理任务' : activeRun ? runStatusName[activeRun.status] : '已完成'}</strong><p>{completedTools > 0 ? `已完成 ${completedTools} 次工具调用` : '正在准备工作步骤'}</p></div></div>{events.filter((event) => event.type !== 'message.delta' && event.type !== 'reasoning.delta').map((event) => <div className="activity-row" key={event.id}><span className="activity-marker" /><div><strong>{eventTitle(event)}</strong>{eventDetail(event) && <p>{eventDetail(event)}</p>}<small>{formatTime(event.createdAt)}</small></div></div>)}</div>)}{tab === 'sources' && <EmptyContext title="尚无引用资料" detail="知识库与网络资料会在研究工作流进入后显示在这里。" />}{tab === 'artifacts' && <EmptyContext title="尚无工作成果" detail="报告、分析、文档和演示会作为可继续编辑的成果出现在这里。" />}</div></aside>;
}

interface SettingsPageProps { tab: SettingsTab; setTab: (tab: SettingsTab) => void; models: ModelProfileSummary[]; modelFilter: ModelProfileSummary['role'] | 'all'; setModelFilter: (filter: ModelProfileSummary['role'] | 'all') => void; activeLanguageModel?: ModelProfileSummary | undefined; onAdd: () => void; onEdit: (model: ModelProfileSummary) => void; onToggle: (model: ModelProfileSummary) => void; onDelete: (model: ModelProfileSummary) => void; appearance: AppearancePreference; resolvedAppearance: ResolvedAppearance; onMode: (mode: AppearanceMode) => void; onScheme: (scheme: ColorScheme) => void; modelMessage: string; }
...
function ModelSettings({ models, modelFilter, setModelFilter, activeLanguageModel, onAdd, onEdit, onToggle, onDelete, modelMessage }: SettingsPageProps): React.JSX.Element { return <section className="settings-section"><div className="settings-heading"><div><p className="eyebrow">模型与能力</p><h2>让每一种工作使用合适的模型</h2><p>API Key 仅保存于本机主进程。语言模型会用于当前任务，视觉与嵌入能力将在对应工作流启用。</p></div><button className="primary-button" onClick={onAdd}>＋ 添加模型</button></div>{modelMessage && <p className="inline-message">{modelMessage}</p>}<div className="filter-bar">{(['all', 'language', 'vision', 'embedding'] as const).map((role) => <button className={modelFilter === role ? 'active' : ''} key={role} onClick={() => setModelFilter(role)}>{role === 'all' ? '全部' : roleName[role]}</button>)}</div><div className="model-list">{models.length === 0 ? <div className="empty-models"><strong>尚未配置模型</strong><p>添加一个 OpenAI-compatible 服务后，即可从教学链路切换到真实模型。</p></div> : models.map((model) => <article className={model.enabled ? 'model-row' : 'model-row disabled'} key={model.id}><div className="model-role-icon" aria-hidden="true">{model.role === 'language' ? '文' : model.role === 'vision' ? '图' : '嵌'}</div><div className="model-main"><div><strong>{model.name}</strong>{activeLanguageModel?.id === model.id && <span className="current-badge">当前工作模型</span>}</div><p>{roleName[model.role]} · {model.provider} · {model.model}</p><small>{model.apiKeyConfigured ? '已配置凭据' : '未配置凭据'} · {model.enabled ? '已启用' : '已停用'}</small></div><div className="model-actions"><button onClick={() => onEdit(model)}>编辑</button><button onClick={() => onToggle(model)}>{model.enabled ? '停用' : '启用'}</button><button className="danger-text" onClick={() => onDelete(model)}>删除</button></div></article>)}</div></section>; }
