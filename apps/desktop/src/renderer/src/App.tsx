...
function ArtifactPage({ artifacts, selected, onSelect, onSave, onBack }: { artifacts: ArtifactSummary[]; selected: ArtifactDetail | undefined; onSelect: (artifact: ArtifactSummary) => void; onSave: (artifact: ArtifactDetail, title: string, content: string) => Promise<void>; onBack: () => void }): React.JSX.Element {
  ...
  useEffect(() => { setEditing(false); setError(''); setTitle(selected?.title ?? ''); setContent(selected?.content ?? ''); }, [selected?.id, selected?.currentVersionId]);
  if (selected) return <section className="artifact-detail-page"><header><button className="back-button" onClick={onBack}>‹ 成果</button><div className="artifact-heading"><div><p className="eyebrow">Markdown · v{selected.versionNumber}{selected.origin === 'user-edit' ? ' · 人工修订' : ''}</p><h1>{selected.title}</h1><p>{selected.origin === 'user-edit' ? '这是人工修订版本；此前版本仍可回溯。' : '来自一次任务运行，可在后续继续修订并形成新版本。'}</p></div>{!editing && <button className="primary-button artifact-edit-button" onClick={() => setEditing(true)}>编辑此版本</button>}</div></header>{editing ? <form className="artifact-editor" onSubmit={(event) => { event.preventDefault(); setError(''); void onSave(selected, title, content).then(() => setEditing(false)).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : '保存修订失败。')); }}><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} required /></label><label>Markdown 内容<textarea value={content} onChange={(event) => setContent(event.target.value)} rows={20} required /></label>{error && <p className="artifact-editor-error">{error}</p>}<footer><span>保存后会创建 v{selected.versionNumber + 1} 人工修订版本。</span><div><button type="button" className="secondary-button" onClick={() => { setEditing(false); setError(''); setTitle(selected.title); setContent(selected.content); }}>取消</button><button type="submit" className="primary-button">保存新版本</button></div></footer></form> : <article className="markdown-preview"><pre>{selected.content}</pre></article>}</section>; return <section className="completed-work-page"><header><div><p className="eyebrow">成果</p><h1>可继续工作的交付物</h1><p>Markdown 是第一种可版本化的成果。后续研究报告、Word、Excel 和 PPT 会接入同一条 Artifact 链路。</p></div><span className="work-count">{artifacts.length} 项</span></header>{artifacts.length === 0 ? <EmptyPage eyebrow="尚无成果" title="将一次完成的回复保存为成果" detail="成果不同于运行记录：它会关联任务、来源运行与版本，方便后续继续修改和导出。" /> : <div className="completed-work-list">{artifacts.map((artifact) => <button className="completed-work-card" key={artifact.id} onClick={() => onSelect(artifact)}><span className="completed-work-icon markdown" aria-hidden="true">MD</span><div><strong>{artifact.title}</strong><p>Markdown · v{artifact.versionNumber}{artifact.origin === 'user-edit' ? ' · 人工修订' : ''} · 更新于 {formatTime(artifact.updatedAt)}</p></div><span aria-hidden="true">›</span></button>)}</div>}</section>; }
