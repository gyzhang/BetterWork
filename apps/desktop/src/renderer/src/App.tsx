...
export function App(): React.JSX.Element {
  ...
  const startNewTask = (): void => { activeRunIdRef.current = undefined; activeTaskIdRef.current = undefined; setActiveRunId(undefined); setActiveTask(undefined); setTaskRuns([]); setEvidence([]); setEvents([]); setPrompt(''); setView('work'); };
  const startRun = async (): Promise<void> => {
    ...
    activeRunIdRef.current = result.runId; setActiveRunId(result.runId); setEvents([]); setContextTab('process'); setContextOpen(true); await refreshRuns(); await refreshTaskRuns(task.id); await refreshTasks();
  };
  ...
  const selectTask = async (task: RecentTaskSummary): Promise<void> => {
    ...
    setActiveRunId(undefined); setActiveTask({ id: task.id, sessionId: task.sessionId, title: task.title }); setPrompt(task.goal); setEvents([]); await refreshTaskRuns(task.id); await refreshEvidence(task.id); setView('work');
  };
...
function ContextPanel({ open, setOpen, tab, setTab, events, evidence, artifacts, activeRun, taskRuns, activityGroups, onSelectRun, onOpenSource }: { open: boolean; setOpen: (open: boolean) => void; tab: ContextTab; setTab: (tab: ContextTab) => void; events: AgentRuntimeEvent[]; evidence: EvidenceSummary[]; artifacts: ArtifactSummary[]; activeRun?: RunSummary | undefined; taskRuns: RunSummary[]; activityGroups: ActivityGroup[]; onSelectRun: (run: RunSummary) => void; onOpenSource: (sourcePath: string) => Promise<void> }): React.JSX.Element {
  ...
  if (!open) return <aside className="context-collapsed" onDoubleClick={handleTitlebarDoubleClick}><button aria-label="展开上下文面板" onClick={() => setOpen(true)}><PanelLeftIcon size={14} /></button></aside>;
