...
export function App(): React.JSX.Element {
  ...
  const saveCurrentArtifact = async (): Promise<void> => {
    ...
    try {
      const artifact = await window.betterwork.artifacts.saveMarkdown({ ...(currentTaskArtifacts[0] ? { artifactId: currentTaskArtifacts[0].id } : {}), taskId: activeTask.id, origin: 'assistant-run', runId: activeRunId, title: activeTask.title, content: assistantText });
  ...
  return <main className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    <section className="main-stage">
      ...
      {view === 'knowledge' && <KnowledgePage documents={knowledgeDocuments} results={knowledgeResults} query={knowledgeQuery} setQuery={setKnowledgeQuery} message={knowledgeMessage} importing={isImportingKnowledge} onImport={() => void importKnowledge()} onSearch={(event) => void searchKnowledge(event)} onOpenSource={openKnowledgeSource} />}
    ...
    {view === 'work' && <ContextPanel open={contextOpen} setOpen={setContextOpen} tab={contextTab} setTab={setContextTab} events={events} evidence={evidence} artifacts={currentTaskArtifacts} activeRun={activeRun} activityGroups={activityGroups} onOpenSource={openKnowledgeSource} />}
...
function KnowledgePage({ documents, results, query, setQuery, message, importing, onImport, onSearch, onOpenSource }: { documents: KnowledgeDocumentSummary[]; results: KnowledgeSearchResult[]; query: string; setQuery: (query: string) => void; message: string; importing: boolean; onImport: () => void; onSearch: (event: FormEvent) => void; onOpenSource: (sourcePath: string) => Promise<void> }): React.JSX.Element {
...
function ArtifactPage({ artifacts, selected, onSelect, onSave, onExport, onBack }: { artifacts: ArtifactSummary[]; selected: ArtifactDetail | undefined; onSelect: (artifact: ArtifactSummary) => void; onSave: (artifact: ArtifactDetail, title: string, content: string) => Promise<void>; onExport: (artifact: ArtifactDetail, versionId?: string) => Promise<{ cancelled: boolean; filePath?: string }>; onBack: () => void }): React.JSX.Element {
  ...
  const visibleVersion = viewingVersion ?? (selected ? { id: selected.currentVersionId, artifactId: selected.id, versionNumber: selected.versionNumber, origin: selected.origin, sourceRunId: selected.sourceRunId, createdAt: selected.updatedAt, content: selected.content, contentHash: selected.contentHash } : undefined);
