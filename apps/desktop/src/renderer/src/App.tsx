...
export function App(): React.JSX.Element {
  ...
  return <main className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    <section className="main-stage">
      ...
      {view === 'artifacts' && <ArtifactPage artifacts={artifacts} selected={selectedArtifact} onSelect={(artifact) => void openArtifact(artifact)} onSave={reviseArtifact} onBack={() => setSelectedArtifact(undefined)} />}
...
function KnowledgePage({ documents, results, query, setQuery, message, importing, onImport, onSearch }: { documents: KnowledgeDocumentSummary[]; results: KnowledgeSearchResult[]; query: string; setQuery: (query: string) => void; message: string; importing: boolean; onImport: () => void; onSearch: (event: FormEvent) => void }): React.JSX.Element {
...
function ArtifactPage({ artifacts, selected, onSelect, onSave, onBack }: { artifacts: ArtifactSummary[]; selected: ArtifactDetail | undefined; onSelect: (artifact: ArtifactSummary) => void; onSave: (artifact: ArtifactDetail, title: string, content: string) => Promise<void>; onBack: () => void }): React.JSX.Element {
  ...
  useEffect(() => {
    setEditing(false); setError(''); setTitle(selected?.title ?? ''); setContent(selected?.content ?? ''); setViewingVersion(undefined);
