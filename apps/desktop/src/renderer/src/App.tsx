...
type SettingsTab = 'models' | 'search' | 'appearance' | 'general';
...
export function App(): React.JSX.Element {
  ...
  return <main className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    <section className="main-stage">
      ...
      {view === 'work' && <>
        <header className="page-header" onDoubleClick={handleTitlebarDoubleClick}><div><p className="eyebrow">工作</p><h1>{activeRun ? '继续完成任务' : '开始一件工作'}</h1></div><div className="page-header-actions"><button className="context-toggle" onClick={() => setContextOpen((open) => !open)}>{contextOpen ? '收起上下文' : '查看上下文'}</button></div></header>
...
function SettingsPage(props: SettingsPageProps): React.JSX.Element { const { tab, setTab } = props; return <div className="settings-layout"><aside className="settings-nav-list"><p className="eyebrow">设置</p><h1>偏好与能力</h1><button className={tab === 'models' ? 'active' : ''} onClick={() => setTab('models')}>模型与能力</button><button className={tab === 'search' ? 'active' : ''} onClick={() => setTab('search')}>搜索</button><button className={tab === 'appearance' ? 'active' : ''} onClick={() => setTab('appearance')}>外观</button><button className={tab === 'general' ? 'active' : ''} onClick={() => setTab('general')}>通用</button></aside><section className="settings-content">{tab === 'models' && <ModelSettings {...props} />}{tab === 'search' && <SearchSettings />}{tab === 'appearance' && <AppearanceSettings {...props} />}{tab === 'general' && <section className="settings-section"><p className="eyebrow">通用</p><h2>工作偏好</h2><div className="setting-placeholder"><strong>通用设置将在后续阶段开放</strong><p>工作目录、语言、数据与更新设置会在这里统一管理。</p></div></section>}</section></div>; }
...
function SearchSettings(): React.JSX.Element {
  ...
  const refresh = async (): Promise<void> => setEngines(await window.betterwork.searchEngines.list());
  useEffect(() => { void refresh(); }, []);
  const save = async (): Promise<void> => {
    try {
      await window.betterwork.searchEngines.save({ provider: 'baidu_qianfan', apiKey, webTopK, enabled: true });
      setApiKey('');
      setMessage('搜索配置已保存，智能体可以在任务中联网搜索并标注来源。');
      await refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : '保存失败，请检查配置。'); }
  };
  const test = async (): Promise<void> => {
    setMessage('正在连接搜索服务…');
    try {
      const result = await window.betterwork.searchEngines.test({ provider: 'baidu_qianfan', apiKey, webTopK });
      setMessage(result.message);
      await refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : '连接测试失败。'); }
  };
  return <section className="settings-section search-settings"><div className="settings-heading"><div><p className="eyebrow">搜索</p><h2>为智能体接入联网搜索</h2><p>API Key 仅保存于本机主进程。启用后，智能体在任务需要时会搜索互联网，并给过程与成果标注网页来源。</p></div></div>{message && <p className="inline-message">{message}</p>}<div className="search-form"><label>搜索引擎<select defaultValue="baidu_qianfan"><option value="baidu_qianfan">百度千帆 AI 搜索</option><option disabled>更多搜索引擎即将支持</option></select></label><label>API Key<input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={configured?.apiKeyConfigured ? '留空则保持原有凭据' : '粘贴服务商控制台生成的 API Key'} /></label><details><summary>高级参数</summary><label>网页结果数量 top_k<input type="number" min={1} max={20} value={webTopK} onChange={(event) => setWebTopK(Math.min(20, Math.max(1, Number(event.target.value) || 10)))} /></label></details><div className="search-actions"><button type="button" className="secondary-button" onClick={() => void test()}>测试连接</button><button type="button" className="primary-button" onClick={() => void save()}>保存并启用</button></div></div><p className="search-status">{configured ? <>当前状态：{configured.enabled ? '已启用' : '已停用'} · {configured.apiKeyConfigured ? '已配置凭据' : '未配置凭据'} · <span className={`connection-status ${configured.connectionStatus}`}>{connectionStatusName[configured.connectionStatus]}</span></> : '当前状态：未配置。保存并启用后，智能体即可联网搜索。'}</p></section>;
}
