...
4. 常用参考保留原始内容哈希和来源工作区。任务材料校验继续拒绝过期修订；跨工作区 ArtifactVersion 只有在 `expert-reference` 或显式全局搜索来源下才允许进入任务，其他路径仍返回工作区不匹配。
...
这项设计让稳定参考材料和当期事实分开，既支持“召唤即开始工作”，又保留逐任务可审计、可收缩的材料范围。后续如需按 Expert×Workspace 管理默认参考，应新增协议和迁移 ADR，不把当前引用字段扩展成隐式全局材料权限。
