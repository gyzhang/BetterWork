...
- 实施：E30 定案；E31 已实现存储、作用域检索、运行注入和受管 Markdown 投影；E32 已实现管理界面、对话确认和当前任务排除，并完成 E3 最小闭环验收。
...
## 后续提案的关系（2026-09-22 追加，尚未生效）

[ADR-0026 工作型记忆的召回、来源治理与提炼边界](0026-work-centered-memory.md) 仍记为 **Proposed**（接受状态待用户确认），但其技术方案已按用户 2026-09-22 的开工指令实施。本记录的状态、接受历史与 E30–E32 的实施/验收记录**均不变**；四种作用域、人工确认优先、投影可重建等约束继续有效，ADR-0026 只细化其召回、来源治理与提炼边界。

该提案声明将细化本文的来源、确认与失败语义，并**拟替代**以下三处条款：§4 中把范围优先级置于内容相关性之前的取候选顺序、§4/§失败与恢复语义中「6,000 个 Unicode 字符」「超出 2,000 字符」未指明计数单位的表述、§4 把「选中」与「实际注入」合为一谈的解读；同时拟补足 §5 范围收缩下缺失的安全重放要求（材料替换/换版本、记忆过期与替代、传递依赖与连续安全后缀）。具体条款对照见该 ADR「与其他 ADR 的关系」一节与其产品设计、实施契约链接。
