...
4. 所有解析都消费 E21 的受管只读 `InputSnapshot`，不读取原始路径；解析结果是临时 Run 输入，不登记为 Artifact。工具/服务 API 统一返回 `sourceSnapshotId`、`format`、`locator`、结构化内容和限制/警告。
...
type OfficeParseResult = {
  ...
  format: string;
  sections: Array<{
    locator: string; // slide:1, slide:1/table:1, sheet:经营数据!A1:B8, row:2
...
解析器只返回材料范围内的数据；跨快照读取和未选择材料由 Main 的 TaskContext 范围校验拒绝。Evidence/RunMaterialRead 使用同一 `locator` 和内容哈希记录实际读取。
