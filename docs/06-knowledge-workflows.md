...
> 首个 PPT Skill 样本采用规范化 SVG → 原生 DrawingML → 公司模板合并的现有管线，阶段 A 需先跑通最小样例，阶段 C 再接入完整业务流程。见 [样本与边界](reviews/2026-09-08-skill-runtime-boundary.md)。下文 PresentationModel 是设计参考，不应成为强制重写外部 Skill 的导入条件。

## 0. 已确认的首条真实工作路径（2026-09-08）

「集团智能体场景落地」由同一专家调用多个 Skill 完成：结合平台设计、开发、规划和功能材料，调研目标业务及官方要求，提取场景，评估平台适配和实施方案，使用公司模板形成可编辑 PPT。用户最后在办公软件中人工调整并用于汇报。

必须停下来讨论的节点：① 理解问题与工作计划；② 网络调研完成；③ 调研报告大纲；④ 调研报告完成；⑤ PPT 大纲；⑥ PPT 完成后的修改与调整。阶段内部自主推进，方向性缺口可提前询问；用户可要求补充研究或返回前面阶段。

这是已确认的产品流程，尚未实现。不得仅凭提示词声称已有可靠等待/恢复能力；采用何种 Run/Step 协议另立 ADR。下文研究与 Office 技术方案是后续设计参考，具体切片见 [修订稿](reviews/2026-09-08-product-scope.md)。
