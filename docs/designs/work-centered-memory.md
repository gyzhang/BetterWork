...
这四个口径当前只有部分有数据源，必须按现状说清而不是含糊承诺：分子侧，确认候选数可从 `memory_operations` 的 `set-status` 回执（`governanceAction='confirm'`）读出，模型耗时与 usage 从 `memory_extraction_jobs` 的 `started_at`／`finished_at`／`usage_json` 读出真实返回值；分母侧，「已展示候选数」没有任何生产者——[任务板 §13.3](../development/tasks-memory.md) 排除了新增第三方埋点，界面也没有展示上报通道，因此候选采纳率与建议负担当前不可计算。重复纠正次数与复用满意度同样只有口径、没有采集入口。本期不实现这些指标，也不得为了凑分母补遥测；要采集必须先另立设计并单独授权。
