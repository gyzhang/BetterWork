...
- 状态：**Accepted / 已定稿，E1–E4 与 E50–E54 已实现，E55–E56 partial**。2026-09-14 用户完成 v0.2 评审并要求落实设计、形成开发计划；本稿产品关系、交互与 E1–E5 范围正式生效。已完成部分均按开发计划提交并推送，未指定的后续实施技术在对应开发卡中定案，不再重复请求同一设计批准。
- 执行入口：[开发计划与任务板](../development/tasks-experts.md)；配套 [ADR-0014](../adr/0014-expert-context-and-material-binding.md) 已接受。本文“建议默认值”按已通过评审的默认方案执行；E55 的真实业务旅程、E56 的签名安装与完整安装态验收仍以任务板记录为准。
...
材料用途由专家预设、添加入口与当前要求提供默认判断，有歧义时由专家询问。用户可在材料详情调整，不必为每个附件逐项填写。主要用途为：**规则口径 / 本期输入 / 历史对比 / 结构参考 / 模板 / 背景参考 / 其他**，可写简短备注。“其他”表示本次材料未指定上述语义，不应被系统擅自解释为数据、规则或模板。同一个文档不重复绑定；兼有用途写入备注。
