...
## 2026-09-15 材料事实硬门禁实现

- `RunService` 为有选定材料的 Run 建立事实账本：成功读取的文本、成果、Office 片段、知识检索结果和 `analyze_business_metrics` 的确定性结果进入账本；最终输出中的金额、百分比和客户/用户数量必须能在原始材料或允许的确定性计算结果中找到。客户数只接受材料原数，不接受跨期间差值或合计。
- 材料范围 Run 若没有任何成功材料读取足迹，或最终数字主张越界，会在发布 `run.completed` 前转为可解释的 `run.failed`，错误要求重新读取材料并把缺失数据写为“材料未提供”。这避免“只输出准备读取”或沿用历史助手数字被当作完成结果。
- 新增回归覆盖：材料读取足迹缺失失败、选定材料读取后正常完成、客户数 `3家` 越界失败；`run-service.test.ts` 定向 42 项通过，`npm run verify` 通过（77 个测试文件 / 567 项测试 / Electron build）。
- 当前桌面复测中，Run `842445fc-7bf5-4c02-b45c-d1b9cbb0aa7e` 与 `6792caad-1704-46f5-a6dc-def6e2c726db` 暴露了模型只输出“正在读取”而未发出工具调用的空跑形态；这两次发生在硬门禁代码重新加载前，未计入业务成功证据。门禁现在会将同样形态收口为失败。已有真实成功 Run `31813b3b-a6dc-4306-bd08-d5f439d0e747` 和事实边界复测 Run `b67a7b4d-9863-405c-9b86-7647539d31dc` 仍作为材料读取与输出样本，但不据此把 E55 升级为通过。
- E55/E56 继续保持 `partial`：真实业务 MCP 连接、连续两期在硬门禁下的成功运行和 Developer ID 签名安装仍未完成；本阶段只交付应用层数字事实门禁与读取足迹收口。
