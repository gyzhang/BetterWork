...
## 2026-09-15 硬门禁后真实材料成功路径

- 重启开发态 Electron 后，在同一专家和三份已选脱敏材料范围内启动 Run `65000f96-d031-4298-a234-916f41d9c40e`。SQLite 状态为 `completed`，`run_material_reads` 登记了 `financial-rules.md`、`2026-09-report.md`、`2026-10-data.md` 三个 `read` 足迹。
- 事件顺序确认先完成 `read_text_file ×3`，再完成 `analyze_business_metrics ×1`；分析工具返回收入 130、上期 120、预算 125、变化额 10、预算差异额 5、变化率 0.083333…、预算差异率 0.04。最终 `run.completed` 输出只使用这些材料或确定性结果，缺失的费用预算、延期回款金额、客户总数和未来预测均写为材料未提供/不可推断，没有出现跨期间客户数合计。
- 这次运行证明硬门禁代码在真实窗口中不会阻断合规材料路径，并把读取足迹、工具结果和最终摘要串成可回溯证据。它仍只覆盖一期，未配置真实业务 MCP，也未完成取消后连续两期成功与 Developer ID 安装，因此 E55/E56 继续保持 `partial`。

## 2026-09-15 定性事实硬门禁

- 数字账本之外，`RunService` 现在对“已续约”“已流失”“平均客单价”“续约率/流失率”和客户总数等材料依赖状态建立短语账本。材料没有提供这些状态时，最终输出中的肯定式主张会在 `run.completed` 发布前失败；“材料未提供”“不可推断”“待确认”等缺失说明仍可通过。
- 新增回归 Run：材料只写“续约意向待确认”，模型读取后输出“该客户已续约”，Run 被收口为 `run.failed`，错误明确标记“材料未提供的定性事实（已续约状态）”。全量 `npm run verify` 通过（77 个测试文件 / 568 项测试 / Electron build）。
- 这是一组针对当前经营分析样本的保守边界词，不把一般风险判断或行动建议当成事实；真实 MCP、两期连续运行和更广泛语义审计仍属于 E55 未完成条件。
- 硬门禁代码重载后的真实窗口复测 Run `9cbb50ff-0baf-4de0-944b-eb02b4bac59a` 读取了三份材料；模型在看到材料仅写“续约意向待确认”后主动拒绝输出“该客户已续约”，以材料原文和“系统拒绝”作答并正常完成。该次没有触发 `run.failed`，因此只作为最新代码和模型自我纠偏的证据，不冒充硬门禁失败样本。
- 账本同时接受用户当前请求中明确给出的定性事实，和已有的显式数字输入保持一致；材料未提供且用户也未明确给出的状态仍受门禁约束。
- 定性门禁将“是否已续约”、核实/跟进/建议和条件句视为非断言语境，允许报告提出核查动作；只有脱离这些语境的肯定式状态才会失败。

## 2026-09-15 数字单位边界

- 数字账本为客户/用户数量单独保留原始计数集合；金额、百分比、日期等数字不能再因为数值相同而被当作“家/户/客户”数量。当前请求里的带计数单位数字和带“客户数/客户数量”标签的数字仍按用户明确输入处理。
- 新增回归覆盖：材料只有收入 `130万元`，模型输出 `客户数：130家` 时，Run 在终态前失败；定向 `run-service.test.ts` 44 项通过，全量 `npm run verify` 通过（77 个测试文件 / 569 项测试 / Electron build）。
- 百分比也按独立集合校验：材料只有收入/预算金额时，`130%` 会失败；确定性工具从 130/125 推出的 104% 可通过。全量 `npm run verify` 通过（77 个测试文件 / 570 项测试 / Electron build）。
