...
4. 直接启动 `apps/desktop/dist/mac-arm64/BetterWork.app` 后，macOS 可访问性树显示：专家列表 →“研究分析专家”→“召唤”→工作输入框，并显示“当前专家”。路径没有“返回任务准备”或启动前配置表单；窗口可正常关闭。

## 限制与后续

- 本机没有 Developer ID Application 证书，构建日志明确记录 App 未签名；本记录不能替代签名 DMG 的安装、升级和 Gatekeeper 验收。
- 本次启动的是构建目录中的 App bundle，没有执行把 DMG 拖入 Applications 后再启动的人工安装步骤。
