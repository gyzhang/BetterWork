...
HEAD `fa820a0`；下列「修复后」一栏的命令统一是 `npm run verify`，最近一次全量为退出 0（136 文件 / 1302 测试）。
...
回归先失败的证据、修复后命令/退出码：`artifact-version-executor.test.ts` 4 例（含 general 分支此前零覆盖）；`App.test.tsx` 断言读取失败保留原任务、迟到响应不覆盖。verify 退出 0。
