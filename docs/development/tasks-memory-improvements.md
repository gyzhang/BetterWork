...
HEAD `fa820a0`；下列「修复后」一栏的命令统一是 `npm run verify`，最近一次全量为退出 0（136 文件 / 1301 测试）。
...
回归先失败的证据、修复后命令/退出码：未确认选区曾可提交（App.test.tsx 断言先失败）；`memory-capture.test.ts` 9 例含非 BMP 与重复片段；1a530b3 前失败路径无面板层用例。修复后 verify 退出 0。
