...
HEAD `fa820a0`；下列「修复后」一栏的命令统一是 `npm run verify`，最近一次全量为退出 0（136 文件 / 1304 测试）。
...
回归先失败的证据、修复后命令/退出码：`d8c0bd8` 的资格用例先失败（未到 validFrom 的记录曾可设优先，写入门禁与召回口径不同）；MI09 曾有优先组注入死循环（vitest worker OOM）与 `pinned-rule` 丢适用条件两处实现缺陷被测试抓出。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：界面部分归 MI06，未测。
真实模型调用次数（默认 0）：0
已知限制与未完成项：优先只免「词面命中」一道门槛；旧 v1 快照原样只读。
任务板/日志位置：§4 MI05；docs/logs/2026-09-25.md 16:10／18:12。
下一卡与前置：MI06（依赖本卡）。
提交与推送状态：已提交 `84d2753`、`d8c0bd8`；未推送、未发布。
```
...
实际修改文件与行为：work-centered-memory.integration.test.ts 覆盖 Q1–Q8（8/8 实际 ModelRequest 带目标规则、score=0、reason=pinned-rule、v2）、R1–R8、N1–N8、AC3 重启后 v1 原样可读、AC4 变异验证；本轮把 R5 钉到 Main 区间/哈希、把 N8 钉到真实请求（含对照组，证明词面命中不得改走相关池绕上限）。
