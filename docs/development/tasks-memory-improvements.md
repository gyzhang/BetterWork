...
回归先失败的证据、修复后命令/退出码：写 N8 时暴露 `manual` 来源摘录上限与正文上限冲突，501–2,000 码点长记忆在 create 抛 ZodError——`543872e` 分开两个上限并补协议双侧边界用例与服务层回归，随后 verify 退出 0；此前 `0324da3` 的优先组注入死循环与丢条件缺陷同样先失败后修。
