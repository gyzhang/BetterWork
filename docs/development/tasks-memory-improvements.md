...
HEAD `4de554a`；下列「修复后」一栏的命令统一是 `npm run verify`，最近一次全量为退出 0（136 文件 / 1305 测试）；此后新增 2 条组原子性用例，总数 1307，同一次全量只剩 KM14 warm p95 因本机负载（load 11+，与 20:46 那次同型）红，MI 五个测试文件单独复跑 90/90 绿。
