...
| MI01 | Main 手工来源依赖闭环 | MI00；D1 | done | 提交 `cd36d2a`。`memory-provenance.ts` 新增最终无工具回答＋真实空间归属＋依赖闭包校验，`ResolvedMemorySource.memoryDependencies` 由解析结果传入（原 `memory-service.ts:162` 硬编空数组已消除）；缺快照/审计→`SOURCE_REVIEW_REQUIRED`，缺修订/哈希不符/失效同码，循环→`SOURCE_DEPENDENCY_CYCLE`，超限→`SOURCE_DEPENDENCY_LIMIT`，501 码点摘录→`SOURCE_MISMATCH`；checkpoint 与 artifact-version 分支不再允许空依赖降级。协议层拒绝 `sourceSelector` ＋ `asUserInstruction=true`。证据：`memory-provenance.test.ts` 21 例、`work-centered-memory.integration.test.ts` 真实 SQLite 两例（继承等式/幂等重放/非最终事件/缺审计/零写入），后续补强（提交 `8074a97`）：材料侧 200 依赖上限与「摘录区间为空」各补一条负例，确认超额是整条拒绝而不是截断，空区间不会被当成已保留来源。门槛复核（20:26）：本卡与集成用例都以 `new MemoryService(store, directory)` 构造服务，不注入 reader——来源解析走真实 store reader（Run 事件、准备快照、审计足迹、材料/记忆依赖都从库里读），「只有 fixture 而无完整生产依赖读取不算通过」这条门槛成立；`memory-provenance.test.ts` 里的假 reader 只服务 resolver 单元层，链路级证明在集成用例。全仓 `npm run verify` 通过 131 文件 1225 测试。 |
| MI02 | 回答原文选择与保留来源保存 | MI01；D1 | doing | 自动化通过，UI 待验。提交 `c3a5348`：`lib/memory-capture.ts`（最终回答判定、UTF-16→码点换算、半代理对拒绝、唯一命中定位）、`components/MemoryCaptureSource.tsx` 只读原文重选、`MemoryEditor` 新增 `requireSource`，未确认选区不能提交且不再静默转 manual；捕获范围过滤掉 user/expert 全局。`App.test.tsx` 新增选区保留来源与不提供全局两例，`memory-capture.test.ts` 9 例含非 BMP 与重复片段；`MemoryEditor.test.tsx` 把「未确认选区」的错误落点钉在 `.inline-message.error` 槽，并断言不存在非错误态内联消息（`1a530b3`）；AC3 的界面半段补上失败保留草稿、重试复用同一幂等键与取消不写库三条断言，同时给假 API 的 `memories.create` 补回真实签名（`eff8e65`）。 |
| MI03 | 持久化排除列表与恢复 | MI00；D2；建议在 MI02 后 | doing | 自动化通过，UI 待验。提交 `f914bcc`：新只读通道 `memory:task-exclusions`（不要求 prompt）＋ `TaskMemoryExclusionItem` strict 联合，越范围/不存在统一 `unavailable` 占位；`MemoryRecallService.taskExclusions` 直接投影 TaskContext 顺序、去重、上限 100（`MEMORY_TASK_EXCLUSION_MAX` 与 TaskContext/保存 Schema 共用），写入仍走 `SaveTaskContextRequest`＋CAS，未新建排除表。证据：`task-memory-exclusions.test.ts` 真实 SQLite 4 例（含不泄露他空间正文、修订冲突回 `REVISION_CONFLICT`＋当前修订号）、`ContextPanel.test.tsx` 独立清单在预览失败时仍可恢复；`use-task-memory-exclusions.test.ts` 4 例钉住钩子行为（不要求 prompt、上下文未建立时不请求也不报错、切任务后迟到响应被丢弃、读取失败可重试且与预览错误互不牵连）；面板层再补读取失败落点用例——错误留在「本任务已排除」原位并带重试、已读到的条目不被清掉、该小节无非错误态内联消息（`1a530b3`）；再补排除额度口径：第 101 条排除由 Hook 在本地拦住并给出「先恢复一条」的可操作原因，不发一条注定被 Schema 拒绝、只会被读成上下文冲突的请求（`use-run-memories.test.ts` 现 9 例）。 |
| MI04 | recallPolicy 字段与真实迁移 | MI-M0；D3 | done | 提交 `dd7bb58`。迁移 **v33** 给 `memory_records` 补 `recall_policy TEXT NOT NULL DEFAULT 'relevant' CHECK IN ('relevant','pinned')`；仓储读写映射、追加修订与治理动作均原样携带策略（不被默认值清掉），新建与自动候选固定 relevant。证据：`migrate.test.ts` v33 用例（旧库两次迁移幂等、身份/正文哈希/状态/来源逐行不变、非法枚举被 CHECK 拒、`foreign_key_check` 为空）、`memory-repository.test.ts` 用真实文件库把历史行 forge 成 pinned 后编辑与 expire 仍保留策略、协议枚举用例。另核对回执的持久化形状：`memory_operations.result_json` 只写 `effect`、`committedRevisionIds` 等身份，不内嵌 `MemoryRecord`，所以旧回执读取不需要补策略字段的兼容分支（AC3 的读取适配在此为空，重放用例已覆盖）。真实环境复核（只读 `sqlite3 mode=ro`）：本机开发库 `~/Library/Application Support/@betterwork/desktop/betterwork.db` 的 `schema_migrations` 已有 33 行、`MAX(version)=33`（名为 `MI04 memory recall policy column`，落章于 2026-09-25 17:13:37 CST），`memory_records` 现带 `recall_policy TEXT NOT NULL DEFAULT 'relevant'` 与枚举 CHECK，`PRAGMA foreign_key_check` 为空，现有修订全部 `relevant`、无 NULL。全仓 verify 通过。 |
| MI05 | 优先策略写入与 v2 召回/历史兼容 | MI04 | done | 自动化通过（真实 ModelRequest 断言已在 MI09 落齐）。协议新增 `memory-recall-v2`/algorithmVersion=2、`MEMORY_RECALL_PINNED_ITEM_LIMIT=6`、`MEMORY_RECALL_PINNED_CODE_POINT_BUDGET=2000`、冻结 v1 快照 Schema＋严格 v2 Schema、`pinned-rule` 选择理由，并在 `runMemoryContextSchema` 拒绝「版本与快照不一致」和「v1 快照里出现 pinned-rule」。`MemoryService.update` 落资格门禁：仅已确认、当前生效、用户口径、零材料/零记忆依赖且非事实/经验可 pinned，否则 `INVALID_TRANSITION`；已 pinned 记录被编辑成不合格形状时要求先取消优先。`applyRecallBudgetV2` 按优先池→偏好池→相关池分配，keep-both 分量整体进出、优先组排序不用 updatedAt 偏袒，新 Run 写 v2。证据：`memory-recall-v2.test.ts` 10 例（零词面命中仍入选且 score 如实为 0、6/7 条与 2,000/2,001 码点边界、三分量整组进整组出、排除项不能穿透、真实 SQLite preview 快照为 v2）。资格补口（`d8c0bd8`）：五条拒绝原因逐条有用例，并修出实现缺陷——写入门禁原先只看 `deriveEffectiveStatus`，未到 `validFrom` 的规则也能设优先，而召回按 `isMemoryEffectiveAt` 判定不会带入；现两侧口径一致，回归用例先失败后通过。AC6「审计失败零发包」此前只在 gate 单元层用假 sink 证过，现补生产链路集成用例（`603e1cd`）：注入真实 `runMemoryContexts.markRequestPrepared` 抛错，断言零 Provider 请求、审计停在 `selected`、Run 终态 `failed`；变异验证吞掉两处审计写入照常发包时用例如期变红（0→1），还原转绿。组原子性另做变异探针（`4de554a`）：去掉 `applyRecallBudgetV2` 的组级判定、改按逐条上限塞入后，4 条用例变红（既有的整组落选用例与集成 N8 各抓到一条成员漏进），并补出既有用例没走到的两条分支——优先池只剩一条名额时装不下整个分量、成员逐条合格但合计超池预算。AC6 三句逐条对账：①「revision 异步准备中变化→不发旧数据」由 `resolveRunContext`（run-service.ts:1516–1522）的 TaskContext 期望修订 CAS 在 Run 起点拦截，配 R7 改修订后旧运行 `revisionId` 冻结；②「审计失败零发包」见上述注入用例；③「已活跃 Run 快照不被策略更新改写」是分层结构保证——快照 `selectedItems` 只在准备期由 `recordSelection`（run-service.ts:554）写一次，派发闸门仅更新 phase/时间戳，而 `memory-service.ts` 对 `runMemoryContexts` 零写引用（grep 证实），故策略更新无法触达该表；不为凑行为用例去 contort 派发时序，按已知结构保证记此边界。 |
| MI06 | 优先规则管理与原因展示 | MI05；D3 | doing | 自动化通过，UI 待验。记忆列表按记录显示「按相关性选择 / 优先带入」徽标，已确认且非终态的记录给「设为优先带入 / 取消优先带入」开关，走既有 `memory:update`＋新 operationId 与 expectedRevision，不新建页面、开关或全局配额；不合格原因（如事实类）由 Main 的 `INVALID_TRANSITION` 文案直接内联显示，前端不另写一套判定。说明文案明确「只免词面门槛、不免范围/有效期/排除/来源/冲突门禁、只影响下次运行、不表示模型一定采用」。证据：`MemoryView.test.tsx` 新增 4 例（提交形状与幂等键、pinned 取消、错误落在 `.inline-message.error` 槽且无非错误态内联消息、终态不给开关）。 |
| MI07 | 冲突来源与并存条件可回看 | MI-M0；D4 | doing | 自动化通过，UI 待验。冲突 DTO 改为按 state 判别：`keep-both` 必带 `applicabilityNote`（1–300），`unresolved`/`replaced` 严格不带该字段，缺条件的旧并存裁决降级显示为未裁决而不是补空串（仓储映射见 `memory-operation-repository.ts:328`）。记忆视图并列两侧正文、适用范围与有效期，并在同一位置回显并存适用条件。证据：`packages/agent-protocol/src/index.test.ts` DTO 分支用例、`memory-operation-repository.test.ts` 裁决回传、`memory-service.test.ts` 真实 SQLite 回看。补齐：`use-memories.loadRevision` 复用既有 `memory:get(id,revisionId)`，冲突卡「查看两侧来源」按 pair 的精确修订分别读取，只回显来源类型/修订/依赖计数/摘录前 60 码点，不显示原始事件载荷；另一侧不在当前列表时不发起读取、只显示占位，异步经 `trackAction` 收口。并存条件的超限解释（提交 `4dc2b0c`）：输入框下方常显「已写 N / 300 码点」，超限时落一条内联原因，不再静默禁用「确认两条并存」；`MemoryView.test.tsx` 断言计数、内联错误、超限时不可提交与修正后可提交。仍未做：UI 人工走查。 |
| MI08 | 换期恢复引导与来源专家修正 | MI-M0；D5 | doing | 自动化通过，UI 待验。新增只读通道 `artifact:get-version-executor`（strict 请求 `{artifactId,artifactVersionId}`，响应 `ArtifactVersionExecutorSummary｜null`，general/expert/unavailable 三分支互斥）＋ `resolveArtifactVersionExecutor`：从该精确版本的来源 Run 快照取身份，user-edit 沿同成果前版回溯，断链/循环/缺快照一律 unavailable 且绝不改取 latest；版本不属于所给成果返回 null。`App.tsx` 的「从此版本开始新任务」不再读旧 Task 当前 executor，也不复制来源任务的技能/模型绑定；专家修订已更新时明确提示按当前配置使用，不可用时按原因提示并回退通用助手。证据：`artifact-version-executor.test.ts` 真实 SQLite 4 例（含回溯、缺快照、停用专家、跨成果不泄露）、`App.test.tsx` 两条更新为快照来源断言。补齐（`f79eb6e`）：「历史上下文调整」在解释截断原因后给出三个可返回入口——选择本期材料、查看上期成果版本、到记忆详情保留方法，并写明「引用只固定所选版本、标记参考不等于已读取、不自动摘要旧回答」；界面不提供也不文案化「恢复全部历史」。`ContextPanel.test.tsx` 断言入口存在、跳转参数正确且无恢复全部历史按钮。另按 AC4/AC5 补守卫：来源身份读取失败改为「保留原任务＋一次可重试错误」，重复点击时旧响应作废不覆盖后一次结果（`App.test.tsx` 2 例）。剩余仅人工走查。补口（`8074a97`）：真实 general 运行快照的来源身份断言为 `general`＋精确 sourceRunId，不再与 `unavailable` 混淆（AC5）；AC3 的界面半段补测（`f6aac1a`）：来源专家修订与当前不同时必须说明「新任务使用其当前版本」，且新草稿不带走来源任务的技能选择——原有用例把两个修订设成同一值，正好绕过该分支。剩余仅人工走查。 |
| MI09 | 离线联合回归与效果验收数据集 | MI01–MI08 | doing | 已跑通并计入本卡：Q1–Q8 八种无词面问法 8/8 经真实 RunService＋可注入 Provider 的实际 ModelRequest 带入目标规则、审计里 reason=pinned-rule、score=0、recallVersion=memory-recall-v2；本轮把 §5.0 的另一半也补回来（`678a8ab`）：八种问法逐一同时断言 preview 侧（v2 快照、pinned-rule、分数 0）与真实请求，并在同一次运行里放一条同样合法但未设优先的对照规则，断言它不进选择集、不进请求、落选原因是「与本任务不相关」——否则「免词面」就退化成对所有记忆的放行；再按行对账补三处「条件被悄悄走弱」（`72fc348`）：§5.0 的材料前提改用真实挂上的零重叠材料而不是 `materials: []`（平凡真不算满足前提）、R2 用它自己那一行的问法真跑一次运行、R5 的「依赖一致」补上两次出现之间 material/memory 依赖相等的断言。N 组逐行复核后无需改动（N1 历史侧、N2 不擦依赖、N5 源文件只读、N6 零写入、N7 不挑胜者、N8 组原子都已有断言）。R3（user pinned 无需历史）、R6（一条 pinned＋两条并存整组带入且附适用条件）、R7（改新修订后只注入最新修订，旧 Run 审计的 revisionId 不变）；N1（排除父规则后派生记忆与注入段都不回流）、N2（父记忆删除后派生记忆不注入且**不擦除**依赖登记）、N3（过期优先规则不注入）、N7（未裁决冲突两侧都不注入，不按时间或置信度挑胜者）；AC3 重启重开库后旧 v1 快照原样解析、不回填新字段。同目录另复用 MI01/MI03/MI05 的真实 SQLite 用例覆盖 R5、N6、N8 与 N4 的越范围不泄露。本轮再补：R2/R4（专家与专家工作空间优先按范围带入）、N4（他空间优先规则既不入请求，也只能以 `unavailable` 占位出现在排除投影里，不暴露正文）、N5（派生记忆依赖的材料换版本后 `dependency-unavailable` 拒绝进入请求，且源文件仍只读、内容未被改写）。AC4 变异验证两次并已精确还原（`git status` 仅本卡测试文件变更）：① 把 MI01 的缺审计拒绝改成 `?? 空依赖` → MI01 集成用例失败；② 关掉 MI05 的 pinned 资格门禁 → `memory-recall-v2.test.ts` 的资格用例失败。另按契约 §11 逐条回扫「条款要求 vs 测试锚点」，补出七处原先无断言的条款（`8074a97`：v1/v2 快照与 recallVersion 必须同侧、v1 上下文里伪造 `pinned-rule` 拒绝、v2 缺优先预算字段、预算字面量与任务排除上限 100、材料 200 依赖上限与空摘录区间、来源执行身份 `general` 分支、替代不继承败者 pinned／改写正文不缩减已登记依赖；`d8c0bd8`：未到生效期不得设优先，并修出该实现缺陷）。剩余：MI01–MI08 的人工 UI 记录（走 §6.1 清单），故本卡保持 doing。本轮把矩阵里两条「只在别的层测过」的行钉回本卡：R5 在 Main 的 provenance 解析器上断言重复句第二处按码点落位、摘录 hash 与事件正文 hash 一致（`d5c9e8d`）；N8 在真实请求上先跑对照组证明该记忆词面可召回，再设优先＋判定并存，断言整组不落、进不了请求、落选原因是 `budget` 而非「不相关」，即不许改走相关池绕开优先池上限（`d5c9e8d`）。写 N8 时暴露并修出一个实现缺陷：manual 来源的「摘录」就是正文本身，却按选择器摘录的 1–500 上限校验，导致 501–2,000 码点的合法长记忆在 `create` 里抛 ZodError（既非成功也非领域失败），现按 §11.1 口径把两个上限分开（`543872e`，含协议双侧边界用例与服务层回归）。可复现性：MI 的 5 个测试文件（集成＋召回 v2＋服务＋provenance＋排除钩子，共 87 例）连跑三次全绿，时间相关断言（未到生效期、有效期边界）不随运行时刻抖动。证据：本卡范围内 MI 五文件单独复跑 90/90 绿（含 `603e1cd` 审计注入、`4de554a` 两条组原子性用例）；两次全量 `npm run verify` 唯红为负载下的 KM14 与 v1-scale 时间基准（单跑各自绿、`memory-retrieval.ts` 无生产 diff），全量总数 1307。本卡完成不代表真实模型语义通过。 |
| MI10 | 人工 UI 与真实模型语义验收 | MI09；独立真实模型授权 | blocked | 前置未满足，未开工：人工走查已整理为 [MI 人工验收清单](memory-mi10-checklist.md)（§1–§7 对应本计划 §6.1 八条路径与 MI02/MI03/MI06/MI07/MI08 的 UI 门槛，由光哥在真实窗口逐格记录）；§8 六个成对场景需光哥书面给出模型配置显示名与整数调用预算（下限 14、建议上限 18）后才是可执行状态。当前自动提炼开关与作业状态未经光哥确认，Qoder 不擅自改设置或开始调用。只读预检（本机开发库）：`workspace_memory_settings` 0 行（无空间开启自动建议）、`memory_extraction_jobs` 0 行（无排队/运行中作业），说明前置在本机形态上已满足，但仍需光哥确认将用于验收的工作空间就是这一套配置后才动手。 |
...
### 7.1 逐卡交接（按上面模板填写，事实来源为提交与本地验证输出）

HEAD `f8d2d47`；下列「修复后」一栏的命令统一是 `npm run verify`。加 `4de554a` 两条组原子性用例后全量总数 1307；最近三次全量唯红都在负载下的时间预算基准（KM14 warm p95、v1 scale 排名），单独复跑各自全绿且生产文件无 diff，MI 五个测试文件单独复跑 90/90 绿。未改任何基准阈值。

```text
MI01：done
本卡授权与范围：2026-09-25 持续落地指令覆盖 MI00–MI09 代码与自动化；真实模型调用、推送、发布未授权。
实际修改文件与行为：memory-provenance.ts / memory-provenance-reader.ts / memory-service.ts——来源必须证明完整依赖链（最终无工具回答、真实空间、准备快照与审计齐全、材料与记忆依赖闭包及环/超额拒绝），provenanceFromResolved 不再写字面空数组。
回归先失败的证据、修复后命令/退出码：MI09 AC4 变异实验把「缺审计→SOURCE_REVIEW_REQUIRED」改成 `?? 空依赖`，MI01 集成用例立即失败；还原后 verify 退出 0。
UI 实际动作与结果（未测必须写未测）：本卡为 Main 层，无独立 UI 动作；界面后果由 MI02 走查覆盖（未测）。
真实模型调用次数（默认 0）：0
已知限制与未完成项：跨库/文件系统不宣称原子；外部可用性在使用时重验。
任务板/日志位置：§4 MI01；docs/logs/2026-09-25.md 15:40。
下一卡与前置：MI02（同 MI-M0）。
提交与推送状态：已提交 `cd36d2a`；未推送、未发布；未改动他人文件。
```

```text
MI02：doing（自动化通过，UI 待验）
本卡授权与范围：同上；不改流式渲染、不做自动提炼、不新建富文本编辑器。
实际修改文件与行为：App.tsx 捕获入口与状态、MemoryEditor.tsx（requireSource/来源文案/码点计数）、MemoryCaptureSource.tsx（只读原文＋确认选区）、lib/memory-capture.ts（最终回答定位、唯一命中、代理对不拆分）；捕获作用域排除 user/expert 全局。
回归先失败的证据、修复后命令/退出码：未确认选区曾可提交（App.test.tsx 断言先失败）；`memory-capture.test.ts` 9 例含非 BMP 与重复片段；1a530b3 前失败路径无面板层用例；`eff8e65` 之前 AC3 的「失败留草稿＋重试复用同一幂等键＋取消不写库」在界面层完全没有断言。修复后 verify 退出 0。
UI 实际动作与结果（未测必须写未测）：未测——清单 §1（1.1–1.9）与 §6.1 路径 1–2 待光哥在真实窗口逐格记录。
真实模型调用次数（默认 0）：0
已知限制与未完成项：1.9（500 码点以上长正文）与 1.2/1.3 的选区拒绝只能由人眼确认；`543872e` 修的正是这类只有真机能暴露的上限冲突。
任务板/日志位置：§2 状态表 MI02、§4 MI02；docs/logs/2026-09-25.md 15:40／19:06／19:19。
下一卡与前置：MI03 同为 MI-M0 门槛，两条人工路径走完才算过。
提交与推送状态：已提交 `c3a5348`、`1dc1f0b`、`543872e`；未推送、未发布。
```

```text
MI03：doing（自动化通过，UI 待验）
本卡授权与范围：同上；不新增排除表、不清空 TaskContext、不扩大任务权限。
实际修改文件与行为：协议 `memory:task-exclusions` 只读通道与 strict 联合（visible／unavailable 占位）、preload、use-task-memory-exclusions.ts、ContextPanel.tsx 的「本任务已排除」独立小节；恢复只从排除 ID 集合移除并保留全部绑定字段，上限 MEMORY_TASK_EXCLUSION_MAX=100。
回归先失败的证据、修复后命令/退出码：`task-memory-exclusions.test.ts` 4 例（真实 SQLite）、`use-task-memory-exclusions.test.ts` 4 例（不要求 prompt、无上下文不请求、迟到丢弃、错误独立）；1a530b3 补面板层读取失败落点用例。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：未测——清单 §2（2.1–2.8），即 MI-M0 的「排除→重算→重启→恢复→仍不相关」与「只显示不可查看占位」两条路径。
真实模型调用次数（默认 0）：0
已知限制与未完成项：恢复只恢复参与资格，不保证入选；跨窗口 CAS 冲突只要求刷新，不做自动合并。
任务板/日志位置：§4 MI03、§1 第 3 步门槛；docs/logs/2026-09-25.md 15:40／17:23／18:30。
下一卡与前置：MI-M0 通过后由光哥确认继续；MI06/MI07/MI08 代码已先行落地，不视为门槛已让。
提交与推送状态：已提交 `f914bcc`、`9395c61`、`1a530b3`；未推送、未发布。
```

```text
MI04：done
本卡授权与范围：只做字段与迁移，不开放 pinned 写入（与 MI05 同卡开放）。
实际修改文件与行为：迁移 v33 增 `recall_policy TEXT NOT NULL DEFAULT 'relevant' CHECK IN ('relevant','pinned')`；仓储读写映射、追加修订与治理动作原样携带；新建与自动候选固定 relevant。
回归先失败的证据、修复后命令/退出码：migrate.test.ts 两次迁移幂等＋非法枚举被 CHECK 拒＋foreign_key_check 为空；真实文件库 forge 成 pinned 后编辑/过期仍保留策略。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：本卡无 UI。
真实模型调用次数（默认 0）：0
已知限制与未完成项：`memory_operations.result_json` 不内嵌 MemoryRecord，故旧回执读取无需策略兼容分支（AC3 读取适配为空）。真机复核：本机开发库 schema_migrations 已 33 行、`MAX(version)=33`、外键检查为空（§2 MI04 证据）。
任务板/日志位置：§2 MI04；docs/logs/2026-09-25.md 18:42。
下一卡与前置：MI05。
提交与推送状态：已提交 `dd7bb58`；未推送、未发布。
```

```text
MI05：done
本卡授权与范围：同卡开放策略写入资格校验＋v2 实际召回；不做向量召回、不改历史 recallVersion、Agent Core 不加数据库依赖。
实际修改文件与行为：memory-retrieval.ts 优先池（6 条／2,000 码点）与 `applyRecallBudgetV2`、memory-recall-service.ts 并存分量整体归属与 `pinned-rule`、包装先算与真实分数、协议 v2 快照与 `runMemoryContextSchema` 跨分支拒绝、memory-service.ts 的 pinned 资格五条拒绝原因。
回归先失败的证据、修复后命令/退出码：`d8c0bd8` 的资格用例先失败（未到 validFrom 的记录曾可设优先，写入门禁与召回口径不同）；MI09 曾有优先组注入死循环（vitest worker OOM）与 `pinned-rule` 丢适用条件两处实现缺陷被测试抓出。AC6「审计失败零发包」由 gate 单元层假 sink 升级为生产链路集成用例（`603e1cd`）：注入真实 `runMemoryContexts.markRequestPrepared` 抛错，断言零请求、审计停在 selected、终态 failed；变异吞掉两处审计写入照常发包时用例如期变红（0→1），还原转绿。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：界面部分归 MI06，未测。
真实模型调用次数（默认 0）：0
已知限制与未完成项：优先只免「词面命中」一道门槛；旧 v1 快照原样只读。
任务板/日志位置：§4 MI05；docs/logs/2026-09-25.md 16:10／18:12。
下一卡与前置：MI06（依赖本卡）。
提交与推送状态：已提交 `84d2753`、`d8c0bd8`、`603e1cd`、`4de554a`；未推送、未发布。
```

```text
MI06：doing（自动化通过，UI 待验）
本卡授权与范围：只复用现有详情与布局，不新建导航、不做「模型已阅读」类文案。
实际修改文件与行为：MemoryView.tsx 策略徽标与「设为/取消优先带入」、五条不合格原因由 Main 回传并内联显示、说明「追加修订、只影响下次运行」；预览侧回显 pinned-rule 与预算/冲突落选原因。
回归先失败的证据、修复后命令/退出码：`MemoryView.test.tsx` 18 例含策略提交形状、取消、错误落 `.inline-message.error`、终态不给开关；`d8c0bd8` 的服务层用例先失败。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：未测——清单 §3（3.1–3.8）与 §6.1 路径 5、§6 运行中改策略。
真实模型调用次数（默认 0）：0
已知限制与未完成项：UI 仍对「未到生效期」记录显示开关（Main 拒绝并给原因）；是否在界面预先隐藏留给走查结果决定，不提前扩大改动面。
任务板/日志位置：§4 MI06；docs/logs/2026-09-25.md 16:41／18:30。
下一卡与前置：MI07→MI08。
提交与推送状态：已提交 `f7970d8`、`1a530b3`；未推送、未发布。
```

```text
MI07：doing（自动化通过，UI 待验）
本卡授权与范围：保持冲突算法不变，只补可解释性与条件回传。
实际修改文件与行为：conflicts DTO 按 state 判别（keep-both 必带 1–300 码点说明，缺条件降级为未裁决）、按需精确修订读两侧来源且只回显摘要、MemoryView 并列正文/范围/有效期与并存条件回显。
回归先失败的证据、修复后命令/退出码：`4dc2b0c` 前，超上限只把「确认两条并存」静默禁用（无计数、无原因）；现用例断言「已写 N / 300 码点」＋内联原因＋修正后可提交。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：未测——清单 §4（4.1–4.8）。
真实模型调用次数（默认 0）：0
已知限制与未完成项：另一侧不可查看时不泄露正文也不代人裁决；编辑任一修订后旧裁决失效需重新裁决。
任务板/日志位置：§4 MI07；docs/logs/2026-09-25.md 16:22／16:51／19:19。
下一卡与前置：MI08。
提交与推送状态：已提交 `fd3a413`、`c96e7df`、`4dc2b0c`；未推送、未发布。
```

```text
MI08：doing（自动化通过，UI 待验）
本卡授权与范围：只读通道补真实消费，不重构专家系统、不复制旧任务的技能/模型绑定、不恢复不安全历史。
实际修改文件与行为：协议 `artifact:get-version-executor` strict 三分支、resolveArtifactVersionExecutor（精确版本来源 Run 快照，user-edit 沿同成果前版回溯，断链/循环→unavailable，跨成果→null，绝不取 latest）、App.tsx「从此版本开始新任务」按快照定身份并带请求序号与任务身份双守卫、ContextPanel「历史上下文调整」三个可返回入口。
回归先失败的证据、修复后命令/退出码：`artifact-version-executor.test.ts` 4 例（含 general 分支此前零覆盖）；`App.test.tsx` 断言读取失败保留原任务、迟到响应不覆盖；`f6aac1a` 补专家修订漂移的说明与「不复制来源技能绑定」。verify 退出 0。
UI 实际动作与结果（未测必须写未测）：未测——清单 §5（5.1–5.8）与 §6.1 路径 7。
真实模型调用次数（默认 0）：0
已知限制与未完成项：不提供也不文案化「恢复全部历史」；标记参考不等于已读取。
任务板/日志位置：§4 MI08；docs/logs/2026-09-25.md 16:32／17:13／17:23。
下一卡与前置：MI09（已完成）→ MI10（人工）。
提交与推送状态：已提交 `c4c6832`、`f79eb6e`、`46fba43`、`d5c9e8d`；未推送、未发布。
```

```text
MI09：doing（离线矩阵通过，UI 证据待并入）
本卡授权与范围：真实 SQLite＋可注入 Provider，零外网；不导入真实用户数据库；合成材料匿名虚构。
实际修改文件与行为：work-centered-memory.integration.test.ts 覆盖 Q1–Q8（8/8 实际 ModelRequest 带目标规则、score=0、reason=pinned-rule、v2）、R1–R8、N1–N8、AC3 重启后 v1 原样可读、AC4 变异验证；本轮把 R5 钉到 Main 区间/哈希、把 N8 钉到真实请求（含对照组，证明词面命中不得改走相关池绕上限），并把 AC6「审计失败零发包」从 gate 单元层假 sink 升级为真实 RunService＋仓储的注入用例。
回归先失败的证据、修复后命令/退出码：Q1–Q8 此前只断言请求侧（`678a8ab` 补 preview 与未设优先的对照规则）；`72fc348` 补矩阵逐行对账的三处弱条件（§5.0 材料前提、R2 自身问法、R5 依赖一致）；写 N8 时暴露 `manual` 来源摘录上限与正文上限冲突，501–2,000 码点长记忆在 create 抛 ZodError——`543872e` 分开两个上限并补协议双侧边界用例与服务层回归，随后 verify 退出 0；此前 `0324da3` 的优先组注入死循环与丢条件缺陷同样先失败后修。
UI 实际动作与结果（未测必须写未测）：本卡不产 UI 结论；MI01–MI08 的人工记录仍缺。
真实模型调用次数（默认 0）：0
已知限制与未完成项：离线通过不代表真实工作效果；MI02/03/06/07/08 的 UI 证据需回填后才能收本卡。
任务板/日志位置：§2 MI09、§5 矩阵；docs/logs/2026-09-25.md 17:01／17:06／17:13／19:06。
下一卡与前置：MI10 需 §6.1 走查＋独立模型授权。
提交与推送状态：已提交 `0324da3`、`675d91f`、`a2c95ac`、`d5c9e8d`、`543872e`；变异改动未提交、已精确还原；未推送、未发布。
```

```text
MI10：blocked
本卡授权与范围：未获授权——需要光哥书面给出模型配置显示名与整数调用预算（§6.2 理想下限 14，建议上限 18），并确认验收用工作空间的自动提炼已关闭且无排队/运行中作业。
实际修改文件与行为：无。仅新增 §8.5 证据表载体（12 行场景×两侧＋固定信息＋汇总口径），使授权后可按 §6.3 字段逐行记录。
回归先失败的证据、修复后命令/退出码：不适用（未执行）。
UI 实际动作与结果（未测必须写未测）：未测——清单 §0 造数与 §1–§7 全部由光哥在真实窗口执行；应用已重启至 pid 63823，主进程与渲染进程均为当前代码。
真实模型调用次数（默认 0）：0
已知限制与未完成项：本机开发库只读预检显示 `workspace_memory_settings` 0 行、`memory_extraction_jobs` 0 行（形态上满足前置），但仍需光哥确认所用工作空间；我不改任何开关、不取消他人作业。
任务板/日志位置：§2 MI10（blocked）、§6.1/§6.2/§6.3；docs/logs/2026-09-25.md 17:38／18:42／19:19。
下一卡与前置：本卡即最后一卡；前置不满足则保持 blocked，不缩小范围收尾。
提交与推送状态：文档提交 `fa820a0`；未推送、未发布。
```
