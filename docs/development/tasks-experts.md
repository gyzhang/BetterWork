...
| E14 | 专家管理与按需配置 UI | E13 | done | [专家配置 UI 测试](../../apps/desktop/src/renderer/src/App.test.tsx)：专家列表/详情、独立编辑器、新建与不可变修订保存、Skill 预设、内置工具 allow-list、模型偏好（应用默认或指定语言模型）、常用参考选择、按当前工作空间隔离的关联记忆摘要与“管理记忆”入口、生命周期操作和内置复制均复用 E11 IPC；本阶段新增常用参考持久化与召唤注入回归，完整门禁见日志。 |
...
| E56 | macOS 安装态与整体收尾 | E55 | partial | [安装资源预检记录](../acceptance/2026-09-14-expert-install-preflight.md)：macOS arm64/x64 DMG 构建、两种架构 App Resources 资源校验、资源绝对路径泄漏扫描和打包 App 召唤冒烟通过；发布预检新增 `--signed-app` 严格签名校验入口，签名安装与真实业务旅程仍待执行。 |
