...
| E14 | 专家管理与按需配置 UI | E13 | done | [专家配置 UI 测试](../../apps/desktop/src/renderer/src/App.test.tsx)：专家列表/详情、独立编辑器、新建与不可变修订保存、Skill 预设、内置工具 allow-list、模型偏好（应用默认或指定语言模型）、常用参考选择、生命周期操作和内置复制均复用 E11 IPC；本阶段新增常用参考持久化与召唤注入回归，完整门禁见日志。 |
...
| E55 | 连续两期真实桌面验收 | E54 | partial | [合成两期验收记录](../acceptance/2026-09-14-expert-two-periods.md) 已覆盖两期独立 Task/Session/Run、Expert 修订与 `expert-reference` 历史成果绑定；RunContextSnapshot 直接保存 `expertId + expertRevisionId`。2026-09-15 已在真实窗口核对“专家 → 召唤 → 工作台”及 `run.started → run.failed` 失败反馈；模型端点不可用，真实材料/网页/MCP/取消重启与连续两期成功旅程仍待执行。 |
| E56 | macOS 安装态与整体收尾 | E55 | partial | [安装资源预检记录](../acceptance/2026-09-14-expert-install-preflight.md)：macOS arm64/x64 DMG 构建、两种架构 App Resources 资源校验、资源绝对路径泄漏扫描和打包 App 召唤冒烟通过；签名安装与真实业务旅程仍待执行。 |
