...
| E56 | macOS 安装态与整体收尾 | E55 | partial | [安装资源预检记录](../acceptance/2026-09-14-expert-install-preflight.md)：macOS arm64/x64 DMG 构建、两种架构 App Resources 资源校验、资源绝对路径泄漏扫描和打包 App 召唤冒烟通过；发布预检新增 `--signed-app` 严格签名校验入口，新增 `expert:acceptance-preflight` 检查模型端点与 Developer ID Application 签名身份并有 CLI 回归测试，签名安装与真实业务旅程仍待执行。 |
