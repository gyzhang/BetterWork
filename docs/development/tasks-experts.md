...
| E42 | MCP 配置及专家/任务工具选择 | E41 | done | 设置页支持 stdio 连接新增/编辑/删除/检测；专家修订保存具体 MCP 工具预设；任务资料面板可按连接选择本次工具并持久化到 TaskContext。`run-service.test.ts` 另覆盖选定 MCP 工具进入模型工具目录、完成本地只读替身调用并收口 Run；历史绑定不随发现变化，失效连接会定位为 MCP 工具不可用且仍可从配置中移除；协议/迁移/ExpertService/Renderer 覆盖已补齐。|
