...
> **2026-09-14 起，本页下方表格转为历史规划。** 用户已通过专家与任务材料 v0.2 评审；有效实施顺序、范围、任务卡与唯一状态表见[专家开发计划](tasks-experts.md)，旧 B/C 每项归属见该计划 §5。本页“待审阅、Proposed、另行拆卡”是历史阶段说明，不能再据此要求重复设计或启动另一套任务。

本文保留完整产品路径，防止阶段 A 被误当作首版全部交付。B/C 不与 A 并行；实施前依据实际 A 的接口和验收结果细化任务卡，不提前猜测尚未存在的表/API。A 的详细粒度是后续拆分模板。

## 阶段 B：专家管理与配置

2026-09-14 交互验收补充：B03/B04 应支持“专家页召唤 → 直接对话”，编辑配置与执行入口分开。缺少业务材料时在对话提供添加入口，不生成启动前必填表单；技术预检仍在发送链路执行。详见[设计 v0.2](../designs/experts-and-task-materials.md) §5。
