...
| KM15 | 人工桌面与真实模型效果验收 | KM14；另有操作/模型授权 | P0 | 3 | todo | 2026-09-25 准备轮（光哥授权打包；状态仍 todo，人工与真实模型未跑）。① 打包前置：`npm run dist:mac --workspace @betterwork/desktop` 退出 0，`app.asar:/out/main` 现含 `knowledge-worker.js` 与 `chunks/knowledge-extract-D1R0bF_k.js`（09-15 旧包只有 `index.js`/`skill-guardian.js`，已用 `@electron/asar` 列表核对）；用包内二进制以 `ELECTRON_RUN_AS_NODE=1` 直跑包内 worker：XLSX 提取出 `sheet:季度数据!A2:C2` 与公式缓存值、CSV 逐行 `rows:N-N`、向量扫描按分排序、伪 nonce 返 `WORKER_NONCE_MISMATCH` 且不执行其 shutdown、正常 shutdown 退出码 0——「asar 内 ESM 入口＋external 依赖能否解析」这项零证据风险消除，.app 冷启动导入仍留清单 §13 人工。② 修掉四处会让验收误判的界面缺陷：刷新不再在作业回执时报「已刷新…本地索引」；降级原因收为唯一中文口径（不再把 `index-partial` 等枚举拼进句子），并与勾选清空提示合成一条不再互相覆盖；详情打开时列表刷新与作业终态会重读版本与正文；移除正开着的资料后不再重读幽灵详情。③ 补齐三项原本无界面、无法人工验收的功能：「重建关键词索引」（keyword 作业此前只有服务端路径）、「检查当前列表来源（N）」（按契约 §12 的 200 份上限自动分批）、「最近作业（含已取消）」＋「查看条目」的逐条目阶段/原因（`knowledge:list-jobs`、`knowledge:get-job` 首次有了界面消费者）。④ 键盘与窄窗按线框补齐：Esc 依次收起局部提示、详情子视图、索引面板（确认框打开时不越级），960px 下摘要行、作业行与详情头纵向堆叠。测试：hook 27/27、view 13/13（本轮新增 12 条）；`npm run verify` 退出 0（136 文件 / 1325 测试）。清单已按实际渲染文案逐屏改写并补前置条件表：`knowledge-km15-checklist.md`。仍不列入人工栏：`artifact:declare-sources`/`artifact:get-run-declarations` 在 renderer 零消费者，「模型声明采用」只能由真实 Run 间接观察；历史作业只保留最近 10 条；20,000×4,096 全库级压测未做。真实模型语义栏（清单 §12）与 .app 冷启动走查（§13）待光哥授权与亲自操作。2026-09-26 08:40 前置对账：0.1 已由光哥确认开发实例能起、可操作、无报错；0.3 系统内 3 个模型均可用且只有 1 个向量模型，光哥已在知识页「嵌入模型」下拉核对选中项即该模型，此确认视为 §12 的真实向量调用授权；0.4 定题不阻断 §1–§11/§13，约定开跑前冻结、跑后不回改，草稿题集与合成语料已出在 `docs/development/knowledge-km15-semantic-set.md`＋`fixtures/km15-semantics/`（10 份资料、20 题预标注、判定口径为标注资料进入前 5 即命中），执行载体待光哥在「界面手跑」与「探针脚本（需单独授权读取凭据）」之间选定。 |
