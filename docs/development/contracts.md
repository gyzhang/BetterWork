...
> 本文继续约束阶段 A 与既有 Skill 执行。2026-09-14 接受的专家/任务材料增量见 [ADR-0014](../adr/0014-expert-context-and-material-binding.md)；[开发计划](tasks-experts.md) E10/E20 负责精确增量契约。本页早期“C 独立处理”后续范围按新计划映射执行，不能以阶段 A 的排除项否定已接受的 E 系列需求。
...
- 目录导入/导出为 A 的必需；ZIP 解析不是自动前置。资源原样保留，frontmatter 未知字段往返；产品内部配置通过额外描述文件导出，信任/密钥不导出。「原样保留」指 Skill 自身内容：操作系统资源管理器元数据（`.DS_Store`、`._*`、`Thumbs.db`、`desktop.ini`）不是内容，导入时排除且不计入内容 hash。
