...
| B00-5 | 撤销级联与 B0 验收 | B00-2、B00-4 | done | `run.cancelled` 协议扩展 `reason?: string`、`cancelRunsForSkill` 写入技能名到取消原因、agent-engine 从 `signal.reason` 读取并带入取消事件；[run-service 测试](../../apps/desktop/src/main/services/run-service.test.ts) 新增取消原因包含技能名断言；`npm run verify` 退出 0（52 文件 / 433 测试），2026-09-13 08:44。真实开发窗口的双技能运行与重启后历史 Run 绑定恢复属于人工验收 |
