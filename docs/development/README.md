...
| A20 | 内置目录与依赖制品打包 | A19 | doing | [electron-builder 配置](../../apps/desktop/electron-builder.yml)、[启动注册](../../apps/desktop/src/main/index.ts)、[SkillService 测试](../../apps/desktop/src/main/services/skill-service.test.ts)（6 项新增：开发/安装寻址、缺资源拒绝、用户副本独立、同名并存、无敏感材料）；macOS arm64 unpacked 包静态预检通过，包内资源和路径扫描见[预检记录](package-preflight.md)，2026-09-15；受管 Python/wheelhouse 冷环境准备和签名安装仍待完成 |
