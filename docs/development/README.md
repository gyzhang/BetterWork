...
| B00-5 | 撤销级联与 B0 验收 | B00-2、B00-4 | doing | `run.cancelled` 协议扩展 `reason?: string`、`cancelRunsForSkill` 写入技能名到取消原因、agent-engine 从 `signal.reason` 读取并带入取消事件；工作阶段摘要现在展示撤销原因；[run-service 测试](../../apps/desktop/src/main/services/run-service.test.ts) 与 [activity 测试](../../apps/desktop/src/renderer/src/activity.test.ts) 已覆盖；`npm run verify` 退出 0（73 文件 / 506 测试），2026-09-14。[双 Skill 重启记录](../acceptance/2026-09-14-b0-two-skill-restart.md) 已证明真实窗口选择、同一 Run 两条绑定和重启后 chip 恢复；模型请求因本机 endpoint `fetch failed` 未成功，撤销级联与成功执行仍待补验 |
