...
| A19 | 文件成果 UI、打开与导出 | A18 | done | [ArtifactView 测试](../../apps/desktop/src/renderer/src/views/ArtifactView.test.tsx)、[use-artifact-viewer 测试](../../apps/desktop/src/renderer/src/hooks/use-artifact-viewer.test.tsx)、[IPC 测试](../../apps/desktop/src/main/ipc/register-ipc.test.ts)；`npm run verify` 退出 0（77 文件 / 571 测试），2026-09-15。开发态桌面已核对 PPTX v1 详情、结构校验通过、预览、系统打开、真实导出、PowerPoint 图形/文本可编辑、另存后关闭重开；Markdown v1/v2 切换与历史版本编辑入口也已回归。导出前后固定为用户可写权限，回归测试覆盖保存面板预创建只读目标；[当日记录](../logs/2026-09-15.md) |
| A20 | 内置目录与依赖制品打包 | A19 | done | [electron-builder 配置](../../apps/desktop/electron-builder.yml)、[启动注册](../../apps/desktop/src/main/index.ts)、[SkillService 测试](../../apps/desktop/src/main/services/skill-service.test.ts)（6 项新增：开发/安装寻址、缺资源拒绝、用户副本独立、同名并存、无敏感材料）；macOS arm64 unpacked 包静态预检通过，`Contents/Resources/wheelhouse` 含 8 个锁定 wheel 和许可/大小清单；真实 `SkillDependencyService` managed cold preparation 已建 venv、离线安装并通过 7 项 import 探测，详见[预检记录](package-preflight.md)与[依赖验证记录](dependency-verification.md) §2/§7，2026-09-15。Windows 与 Developer ID 签名安装仍分别保留在 A09/A21 |
...
| B00-3 | 受控弹层基座 | 无 | done | [PopoverMenu 测试](../../apps/desktop/src/renderer/src/components/PopoverMenu.test.tsx)（11 项：焦点进入首项、Esc 关闭、背板点击关闭、ArrowDown/Up 跳过 disabled 且 wrap、Home/End、Enter 选中、disabled 不可选、hint 渲染、ARIA 属性、无 emoji）；样式只消费 `--surface-raised` / `--border` / `--shadow-color-overlay` / `--selection`，命中区 32px；`npm run verify` 退出 0（52 文件），2026-09-13 08:16 |
| B00-4 | Composer `+` 菜单与 chip 条 | B00-1、B00-3 | done | `RunSummary` 协议扩展 `bindings`（不新增 IPC channel）、`SkillExecutionRepository.listBindingSummariesForRun` 联表查询、`ComposerCapabilityPicker` 组件（一级菜单技能/专家/添加文件，技能二级带搜索、多选、blockedReasons 置灰与定位入口）、chip 条替换纯文本提示、试运行入口写入同一状态、切换历史任务恢复绑定集合、运行中禁止修改；[App 测试](../../apps/desktop/src/renderer/src/App.test.tsx) 同步更新 chip 断言；`npm run verify` 退出 0（52 文件 / 432 测试），2026-09-13 08:36 |
| B00-5 | 撤销级联与 B0 验收 | B00-2、B00-4 | doing | `run.cancelled` 协议扩展 `reason?: string`、`cancelRunsForSkill` 写入技能名到取消原因、agent-engine 从 `signal.reason` 读取并带入取消事件；工作阶段摘要现在展示撤销原因；OpenAI 兼容 Provider 与设置页连接探测的失败诊断会显示安全 endpoint 且不泄露 API Key，运行端点也正确处理带 query 的 base URL；MCP 失效连接中的既有工具绑定仍可取消；同一 Run 双 Skill 撤销只产生一次带原因的取消终态；MCP 选择器按 `connectionId + toolId` 隔离同名工具；Expert 运行取消后保留 `RunContextSnapshot` 并可用同一上下文重新启动的回归已加入；[run-service 测试](../../apps/desktop/src/main/services/run-service.test.ts)、[activity 测试](../../apps/desktop/src/renderer/src/activity.test.ts)、[Provider 测试](../../packages/agent-core/src/openai-compatible-provider.test.ts)、[连接探测测试](../../apps/desktop/src/main/services/model-connectivity.test.ts) 与 [MCP 选择测试](../../apps/desktop/src/renderer/src/lib/mcp-selection.test.ts) 已覆盖；专家详情现在显示关联记忆摘要并可进入按专家/工作空间筛选的记忆管理；发布预检 CLI、验收前置 CLI 及其回归测试已加入，E55 Expert×Workspace 记忆读取和跨 Task 历史隔离也已覆盖；`npm run verify` 退出 0（77 文件 / 564 测试），2026-09-15。[双 Skill 重启记录](../acceptance/2026-09-14-b0-two-skill-restart.md) 已证明真实窗口选择、同一 Run 两条绑定和重启后 chip 恢复；模型 endpoint 无响应，真实撤销走查与成功执行仍待补验 |
