...
E20 的契约完成后，E21 已实现知识修订、输入快照和恢复，E22 已实现选择候选、来源校验与草稿持久化，E23 已完成上下文收缩和工具入口过滤，E24 已完成读取足迹与成果输入关系，E25 已把这些事实呈现在 Composer、资料面板和成果复用界面。E25 的候选 IPC 同时接受已有 Task 或当前 Workspace，确保新任务在首条消息前也能添加材料。
