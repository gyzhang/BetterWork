...
落地现状：圆角与阴影用途已按规范执行（普通内容不用阴影，浮层与 Toast 才用）；分组以留白和 1px 分隔线为主，未出现「每个对象都套卡片」。

2026-09-26 复核并整改：知识页工具栏此前没有任何纵向机制，「全部资料」下拉与搜索框 0 间距贴合。`display:flex` + `gap:12px` 已收进 `.page-toolbar` 基类，记忆页的 `display:grid` 覆写与 `.knowledge-search` 自带下边距一并删除，`FieldSelect` 的 `width:100%` 同时去掉（成果页「采用来源」行里同类的标签挤压一并消失）。

同一轮复核也推翻了上一轮（2026-09-05）「间距已全量对齐标尺、仓库中不再有偏离标尺的间距值」的结论：当时 `styles.css` 的 `gap` 仍有 118 处取值偏离标尺（2 / 3 / 5 / 6 / 7 / 9 / 10 / 11 / 14 / 15 / 20px）。本轮按「同档归一格」全部收口——同一档原本就差 1px 的取值必须落到同一格，避免 5 与 6 被就近拆成两档：

| 原值 | 归到 | 这一档是什么 |
| --- | --- | --- |
| 2 / 3 | 4 | 同一行内紧贴的文本对与菜单项列表 |
| 5 / 6 / 7 / 9 | 8 | 图标与文字、标签与控件 |
| 10 / 11 | 12 | 卡片与工具栏里的并列块 |
| 14 / 15 | 16 | 表单与详情区块的纵向堆叠 |
| 20 | 24 | 版心级分栏与标题区 |

收口后 `gap` 取值只剩 4（34 处）/ 8（123 处）/ 12（56 处）/ 16（16 处）/ 24（2 处），偏离 0 处，并由护栏锁定档位。「间距标尺」目前只覆盖 `gap` / `row-gap` / `column-gap`；`margin` 与 `padding` 仍是分散的裸像素值，纳入同一标尺属下一轮改造。
