...
反馈实现必须先按 [UI/UX 体系 §11.5](10-ui-ux-system.md) 路由语义，再选择组件：短时本地操作成功/失败使用现有 `TransientToast`；需要用户在当前表单或对象上采取行动的错误使用页面内联反馈；需要跨页面回看或关联目标的长操作结果才进入消息中心。禁止在 Hook 或页面里另造自动消失计时器、顶部成功横幅或第二套 Toast；对象状态本身能表达结果时，不重复制造全局提示。
