...
export const createWebSearchTool = (search: WebSearch): AgentTool => ({
  name: 'web_search',
  description:
    'Search the public internet with the workspace’s configured search engine. Returns a short answer summary and web page results with titles, URLs, and snippets for citing or further work.',
  inputSchema: {
    type: 'object',
    properties: {
      query: {
        type: 'string',
        description: 'Search keywords or a question to look up on the web.',
      },
    },
    required: ['query'],
  ...
  async execute(rawInput, context) {
    ...
    return {
      ...
      results,
      message:
        results.length === 0 ? '没有搜索到相关网页。' : `搜索到 ${results.length} 条网页结果。`,
    };
  },
});
