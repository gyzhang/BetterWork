  ...
  inputSchema: {
    ...
    properties: {
      ...
      mimeType: {
        type: 'string',
        description:
          'MIME type of the output file (e.g. application/vnd.openxmlformats-officedocument.presentationml.presentation).',
      },
      description: {
        type: 'string',
      ...
      validation: {
        type: 'object',
        description:
          'Validation state for structure, visual, and manualEdit dimensions. Each: pending | passed | failed | not-checked. Structure=failed is rejected.',
        properties: {
          structure: { type: 'string', enum: ['pending', 'passed', 'failed', 'not-checked'] },
          visual: { type: 'string', enum: ['pending', 'passed', 'failed', 'not-checked'] },
          manualEdit: { type: 'string', enum: ['pending', 'passed', 'failed', 'not-checked'] },
        },
        required: ['structure', 'visual', 'manualEdit'],
        additionalProperties: false,
      },
    },
    required: ['executionId', 'outputId', 'title', 'mimeType', 'validation'],
    additionalProperties: false,
  },
