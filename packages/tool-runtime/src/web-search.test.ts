...
describe('createWebSearchTool', () => {
  it('returns bounded web results with an optional summary', async () => {
    ...
    const output = (await tool.execute({ query: '市场动态' }, context)) as {
      query: string;
      message: string;
      summary?: string;
  ...
  it('reports an empty search without a summary', async () => {
    ...
    const output = (await tool.execute({ query: '冷门问题' }, context)) as {
      message: string;
      summary?: string;
    };
  ...
  it('propagates cancellation and rejects invalid input', async () => {
    ...
    const tool = createWebSearchTool(async () => ({ results: [] }));
    await expect(
      tool.execute({ query: '查询' }, { ...context, signal: abortController.signal }),
